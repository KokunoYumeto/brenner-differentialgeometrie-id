#!/usr/bin/env python3
"""Build the deterministic O011 Indonesian semantic HTML reader through Unit 13.
...
Unit 10 HTML baseline, binds the live Unit 11--13 translation and post-
...
def load_live_qa_bindings(root: Path, units: Iterable[int] = range(11, 14)) -> dict[str, Any]:
    result: dict[str, Any] = {}
    for unit in units:
        tag = f"{unit:02d}"
        ...
        for item in translations:
            for key in ("target", "translation_receipt"):
                binding = item[key]
                expect(binding["path"] in evidence_strings, f"Unit {unit} POST QA does not name {binding['path']}")
                expect(binding["sha256"] in evidence_strings, f"Unit {unit} POST QA does not bind {binding['path']}")
        result[tag] = {
...
def generation_contract(root: Path) -> dict[str, Any]:
    return {
        "unit10_baseline": unit10_baseline(root),
        "unit11_13_live_qa": load_live_qa_bindings(root),
...
def reader_css(renderer: Renderer) -> str:
    ...
    return value.replace("O011 HTML v10", "O011 HTML v13", 1)


README_TEXT = f'''Geometri Diferensial dan Manifold Mulus — pembaca HTML hingga Unit 13
...
<head>
  ...
  <meta name="description" content="Pembaca reflowable Bahasa Indonesia untuk Kuliah dan Lembar Kerja 1–13 dari Differentialgeometrie (Osnabrück 2023).">
  <title>Geometri Diferensial dan Manifold Mulus — Pembaca hingga Unit 13</title>
...
<header class="masthead">
  <div class="masthead-inner">
    ...
    <p class="subtitle">Pembaca kumulatif hingga Unit 13</p>
...
<main id="reader">
  <section class="frontmatter" id="tentang-edisi">
    ...
    <p>Pembaca ini menerjemahkan Kuliah 1–13 dan Lembar Kerja 1–13 dari kursus Holger Brenner di Wikiversity berbahasa Jerman. Teks sumber digunakan berdasarkan <a href="https://creativecommons.org/licenses/by-sa/4.0/">CC BY-SA 4.0</a>. Terjemahan ini merupakan karya independen dan bukan edisi resmi atau dukungan dari penulis, Wikiversity, atau Wikimedia Foundation.</p>
  ...
  <section class="backmatter" id="lisensi-dan-provenans">
    ...
    <p>Edisi ini tidak resmi dan tidak menyiratkan dukungan dari Holger Brenner, Wikiversity, para pembuat media, atau Wikimedia Foundation. Identitas sumber, perubahan, QA langsung, batas publik Unit 10, dan hash masukan tersedia pada <a href="manifest.json">manifes deterministik pembaca</a>.</p>
  </section>
</main>
<footer><p>Pembaca semantik reflowable · Bahasa Indonesia · Unit 1–13</p></footer>
</body>
</html>
'''
...
def _stage_cycle(root: Path, staging: Path, contract: dict[str, Any]) -> dict[str, Any]:
    ...
    manifest = {
        ...
        "workflow": WORKFLOW,
        "scope": "O011 cumulative Indonesian semantic HTML reader through Unit 13",
        ...
        "unit10_baseline": contract["unit10_baseline"],
        "unit11_13_live_qa": contract["unit11_13_live_qa"],
...
def assert_output_path(root: Path, output: Path) -> Path:
    ...
    try:
        ...
        raise RuntimeError("v13 output must remain beneath output/html") from exc
    expect(bool(relative.parts), "v13 output may not replace output/html itself")
    expect(output != (html_root / "unit-10").resolve(), "the public Unit 10 output is immutable")
    return output
...
def build(root: Path, output: Path, replace: bool) -> dict[str, Any]:
    ...
    if output.exists():
        ...
        expect(replace, f"output already exists (use --replace for this exact v13 directory): {output}")

    contract = generation_contract(root)
    output.parent.mkdir(parents=True, exist_ok=True)
    first = Path(tempfile.mkdtemp(prefix=".html-v13-cycle1-", dir=output.parent))
    second = Path(tempfile.mkdtemp(prefix=".html-v13-cycle2-", dir=output.parent))
    ...
    try:
        ...
        expect(first_inventory == second_inventory, "two complete Unit 13 staging trees are not byte-identical")
        expect(first_manifest == second_manifest, "two Unit 13 staging manifests differ")
        if output.exists():
...
def main() -> None:
    ...
    parser.add_argument("--replace", action="store_true", help="replace only the exact declared v13 output after two complete identical staging cycles")
