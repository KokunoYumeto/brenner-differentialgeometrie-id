#!/usr/bin/env python3
"""Build the deterministic O011 Indonesian semantic HTML reader through Unit 19.
...
Unit 13 HTML baseline, binds the live Unit 14--19 translation and post-
...
UNIT_TITLES = {
    **v10.UNIT_TITLES,
    ...
    16: "Manifold Riemann",
    17: "Perhitungan pada Manifold Riemann",
    18: "Isometri dan Permukaan Hiperbolik",
    19: "Pemetaan Weingarten dan Kelengkungan Gauss",
}
...
def load_live_qa_bindings(root: Path, units: Iterable[int] = range(14, 20)) -> dict[str, Any]:
    result: dict[str, Any] = {}
    for unit in units:
        tag = f"{unit:02d}"
...
def generation_contract(root: Path) -> dict[str, Any]:
    return {
        ...
        "unit14_19_live_qa": load_live_qa_bindings(root),
...
class Renderer(v10.Renderer):
    ...
    def _figure(self, body: str, state: SurfaceState) -> str:
        ...
        old_alt = str(occurrence.get("alt") or "")
        normalized_alt = " ".join(old_alt.split())
        if normalized_alt != old_alt:
            old_attribute = f'alt="{html.escape(old_alt, quote=True)}"'
            new_attribute = f'alt="{html.escape(normalized_alt, quote=True)}"'
            expect(old_attribute in rendered, "rendered figure alt attribute is not uniquely normalizable")
            rendered = rendered.replace(old_attribute, new_attribute, 1)
            occurrence["alt"] = normalized_alt
            if occurrence.get("caption_supplied"):
                occurrence["caption_text"] = normalized_alt
...
@media(max-width:46rem){.unit,.lecture,.worksheet,.semantic-block,.exercise,.source-solution,.proof{min-width:0;max-width:100%}.math.inline{display:inline-block;max-width:100%;overflow-x:auto;overflow-y:hidden;vertical-align:middle}}
'''


def reader_css(renderer: Renderer) -> str:
    value = v10.CSS + REFLOW_CSS
    if renderer.has_animated_media:
        value += v10.ANIMATED_MEDIA_CSS
    if renderer.linked_media_used:
        value += LINKED_MEDIA_CSS
    return value.replace("O011 HTML v10", "O011 HTML v19", 1)


README_TEXT = f'''Geometri Diferensial dan Manifold Mulus — pembaca HTML hingga Unit 19
...
<head>
  ...
  <meta name="description" content="Pembaca reflowable Bahasa Indonesia untuk Kuliah dan Lembar Kerja 1–19 dari Differentialgeometrie (Osnabrück 2023).">
  <title>Geometri Diferensial dan Manifold Mulus — Pembaca hingga Unit 19</title>
  <link rel="stylesheet" href="assets/reader.css">
  <script id="mathjax-config">{MATHJAX_CONFIG}</script>
  <script defer src="{MATHJAX_URL}"></script>
  <script id="deep-link-stabilizer">(()=>{{let observer,debounce,watcherStop;const align=()=>{{if(!location.hash)return;const target=document.getElementById(decodeURIComponent(location.hash.slice(1)));if(!target)return;const root=document.documentElement;const previous=root.style.scrollBehavior;root.style.scrollBehavior="auto";target.scrollIntoView({{block:"start"}});root.style.scrollBehavior=previous;}};const watchLayout=()=>{{if(observer)observer.disconnect();clearTimeout(debounce);clearTimeout(watcherStop);if(!document.body||!("ResizeObserver" in window))return;observer=new ResizeObserver(()=>{{clearTimeout(debounce);debounce=setTimeout(align,80);}});observer.observe(document.body);watcherStop=setTimeout(()=>{{observer.disconnect();align();}},30000);}};const settle=()=>{{align();requestAnimationFrame(align);setTimeout(align,250);setTimeout(align,1000);setTimeout(align,3000);watchLayout();}};addEventListener("load",settle,{{once:true}});addEventListener("hashchange",settle);if(document.fonts)document.fonts.ready.then(settle);}})();</script>{animated_media_head}
</head>
<body>
...
<header class="masthead">
  <div class="masthead-inner">
    ...
    <p class="subtitle">Pembaca kumulatif hingga Unit 19</p>
...
<main id="reader">
  <section class="frontmatter" id="tentang-edisi">
    ...
    <p>Pembaca ini menerjemahkan Kuliah 1–19 dan Lembar Kerja 1–19 dari kursus Holger Brenner di Wikiversity berbahasa Jerman. Teks sumber digunakan berdasarkan <a href="https://creativecommons.org/licenses/by-sa/4.0/">CC BY-SA 4.0</a>. Terjemahan ini merupakan karya independen dan bukan edisi resmi atau dukungan dari penulis, Wikiversity, atau Wikimedia Foundation.</p>
...
<footer><p>Pembaca semantik reflowable · Bahasa Indonesia · Unit 1–19</p></footer>
</body>
</html>
'''
...
def _stage_cycle(root: Path, staging: Path, contract: dict[str, Any]) -> dict[str, Any]:
    ...
    manifest = {
        ...
        "workflow": WORKFLOW,
        "scope": "O011 cumulative Indonesian semantic HTML reader through Unit 19",
        ...
        "unit14_19_live_qa": contract["unit14_19_live_qa"],
...
def assert_output_path(root: Path, output: Path) -> Path:
    ...
    try:
        ...
        raise RuntimeError("v19 output must remain beneath output/html") from exc
    expect(bool(relative.parts), "v19 output may not replace output/html itself")
    immutable_outputs = {
        (html_root / "unit-10").resolve(),
        (html_root / "unit-13").resolve(),
        (html_root / "unit-16").resolve(),
    }
    expect(output not in immutable_outputs, "published Unit 10/13/16 outputs are immutable")
    return output
...
def build(root: Path, output: Path, replace: bool) -> dict[str, Any]:
    ...
    if output.exists():
        ...
        expect(replace, f"output already exists (use --replace for this exact v19 directory): {output}")

    contract = generation_contract(root)
    output.parent.mkdir(parents=True, exist_ok=True)
    first = Path(tempfile.mkdtemp(prefix=".html-v19-cycle1-", dir=output.parent))
    second = Path(tempfile.mkdtemp(prefix=".html-v19-cycle2-", dir=output.parent))
    ...
    try:
        ...
        expect(first_inventory == second_inventory, "two complete Unit 19 staging trees are not byte-identical")
        expect(first_manifest == second_manifest, "two Unit 19 staging manifests differ")
        if output.exists():
...
def main() -> None:
    ...
    parser.add_argument("--replace", action="store_true", help="replace only the exact declared v19 output after two complete identical staging cycles")
