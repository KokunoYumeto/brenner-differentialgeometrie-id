#!/usr/bin/env python3
"""Bounded cumulative PDF structural/accessibility QA through Unit 13.

The output PDF identity is learned only from the passing two-clean-cycle build
receipt, never hardcoded while Unit 13 is unfinished. Stable structural facts
...
BUILD_REL = "qa/unit-13/build.json"
MEDIA_CONFIG_REL = "source/unit_media.json"
OUTPUT_REL = "qa/unit-13/pdf_structural_qa.json"
DRIVER_REL = "build/generated/through-unit-13-driver.tex"
...
EXPECTED_METADATA = {
    ...
    "/Title": "Geometri Diferensial dan Manifold Mulus Pembaca kumulatif hingga Unit 13",
...
EXPECTED_EXERCISE_TOTAL = 287
EXPECTED_SOLUTION_TOTAL = 35
...
REQUIRED_READER_PROSE = {
    ...
    "cumulative boundary": r"\bPembaca kumulatif hingga Unit 13\b",
...
def verify_build(blockers: list[str]) -> dict[str, object]:
    ...
    data = load_json(path)
    add(blockers, data.get("workflow") == "o011-through-unit13-pdf-build-v1", "wrong build workflow")
    ...
    add(
        blockers,
        prefix_preservation.get("workflow") == "o011-unit13-unit10-prefix-preservation-v2"
    ...
    required = {
        ...
        *[f"qa/unit-{unit:02d}/POST_CORRECTION_MATH_QA.json" for unit in range(11, 14)],
        *[f"qa/unit-{unit:02d}/solution_closure.json" for unit in range(11, 14)],
        *[f"qa/unit-{unit:02d}_media.json" for unit in range(11, 14)],
    }
    for unit in range(11, 14):
        required.update(
            {
    ...
    add(blockers, [row.get("unit") for row in unit_bindings] == [11, 12, 13], "unit binding order/closure mismatch")
    for row in unit_bindings:
        unit = int(row.get("unit", 0))
        if unit not in (11, 12, 13):
            continue
        add(
            blockers,
        ...
        add(
            blockers,
            post.get("source_closure", {}).get("supplied_solution_indices") == EXPECTED_SOLUTION_NUMBERS[unit],
            f"Unit {unit} POST QA supplied-solution closure changed",
        )

    wrapper = data.get("wrapper", {})
    add(blockers, wrapper.get("workflow") == "o011-unit13-wrapper-derivation-v1", "wrong wrapper derivation workflow")
    add(
        blockers,
        wrapper.get("geometry") == {"paper": "A4", "margin": "22mm", "centered": True, "class_option": "oneside"},
        "wrapper geometry contract mismatch",
    )
    add(blockers, wrapper.get("extension_units") == [11, 12, 13], "wrapper unit extension mismatch")
    add(
        blockers,
        wrapper.get("supplied_solutions")
        == {"11": [10, 14], "12": [11, 12], "13": [1, 10, 11, 16, 18, 19, 21, 22]},
...
def verify_reader_content(
    ...
    add(blockers, solution_bookmark_count == 12, f"source-solution bookmark count is {solution_bookmark_count}, expected 12")
    add(blockers, len(records) == 65, f"bookmark count is {len(records)}, expected 65")
...
def verify_media(
    ...
    rows = [row for unit in range(1, 14) for row in units.get(str(unit), {}).get("media", [])]
    ...
    add(blockers, set(units) == {str(unit) for unit in range(1, 14)}, "media configuration does not cover exactly Units 1-13")
    add(blockers, filenames == EXPECTED_CONFIGURED_MEDIA, f"configured media filename closure mismatch: {filenames}")
    add(blockers, len(rows) == 21, f"configured media census is {len(rows)}, expected 21")
    ...
    for unit, expected_names in NEW_MEDIA.items():
        rel = f"qa/unit-{unit:02d}_media.json"
    ...
    return {
        ...
        "expected_configured_media_count": 21,
...
def write_early_failure(output_path: Path, blockers: list[str]) -> int:
    ...
    output_path.write_text(
        json.dumps(
            {
                "schema_version": 1,
                "workflow": "o011-through-unit13-pdf-structural-accessibility-qa-v1",
                "verdict": "FAIL",
                "passed": False,
                "blockers": blockers,
            },
...
def main() -> int:
    ...
    add(blockers, page_count > _prefix.EXPECTED_PAGES, f"Unit 13 cumulative PDF has only {page_count} pages")
    ...
    expected_page_labels = ["1", "i", "ii", "iii", "iv"] + [str(value) for value in range(1, page_count - 4)]
