#!/usr/bin/env python3
"""Anonymously verify the reader-first Unit 5 Zenodo revision and every byte."""
...
LOCAL_FILES = {
    ...
    "FILE_MANIFEST.csv": "output/figshare/FILE_MANIFEST.csv",
    "CHECKSUMS.sha256": "output/figshare/CHECKSUMS.sha256",
}
...
def main() -> None:
    ...
    if set(remote_by_name) != set(LOCAL_FILES):
        raise SystemExit("public six-file inventory mismatch")
    result_files = []
    for name, relative in LOCAL_FILES.items():
        ...
        download = session.get(remote["links"]["self"], stream=True, timeout=300)
