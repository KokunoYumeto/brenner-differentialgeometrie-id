...
$qaDir = Join-Path $laneRoot 'qa\unit-03'
$workDir = Join-Path $laneRoot 'tmp\pdfs\through-unit03-build'
$outputDir = Join-Path $laneRoot 'output\pdf'
$outputPdf = Join-Path $outputDir 'geometri-diferensial-manifold-mulus-hingga-unit-03-id.pdf'
...
$auxiliaryNames = @(
    'through-unit-03.aux', 'through-unit-03.log', 'through-unit-03.out',
    'through-unit-03.pdf', 'through-unit-03.toc', 'through-unit-03.lof',
    'through-unit-03.fls', 'through-unit-03.fdb_latexmk'
)
...
function Invoke-BuildCycle {
    ...
    $built = Join-Path $buildDir 'through-unit-03.pdf'
...
$receipt = [ordered]@{
    schema_version = 1
    workflow = 'o011-through-unit03-pdf-build-v1'
    engine = $engineVersion
    command = 'pdflatex -interaction=nonstopmode -halt-on-error -file-line-error -recorder through-unit-03.tex (three passes per clean cycle; two cycles)'
