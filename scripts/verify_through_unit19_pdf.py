#!/usr/bin/env python3
"""Bounded cumulative PDF structural/accessibility QA through Unit 19.

The output PDF identity is learned only from the passing two-clean-cycle build
receipt, never hardcoded while Unit 19 is unfinished. Stable structural facts
...
BUILD_REL = "qa/unit-19/build.json"
MEDIA_CONFIG_REL = "source/unit_media.json"
OUTPUT_REL = "qa/unit-19/pdf_structural_qa.json"
DRIVER_REL = "build/generated/through-unit-19-driver.tex"
...
EXPECTED_METADATA = {
    ...
    "/Title": "Geometri Diferensial dan Manifold Mulus Pembaca kumulatif hingga Unit 19",
...
EXPECTED_EXERCISE_TOTAL = 394
EXPECTED_SOLUTION_TOTAL = 54
...
REQUIRED_READER_PROSE = {
    ...
    "cumulative boundary": r"\bPembaca kumulatif hingga Unit 19\b",
...
def verify_build(blockers: list[str]) -> dict[str, object]:
    ...
    data = load_json(path)
    add(blockers, data.get("workflow") == "o011-through-unit19-pdf-build-v1", "wrong build workflow")
    ...
    add(
        blockers,
        prefix_preservation.get("workflow") == "o011-unit19-unit10-prefix-preservation-v2"
    ...
    required = {
        ...
        *[f"qa/unit-{unit:02d}/POST_CORRECTION_MATH_QA.json" for unit in range(11, 20)],
        *[f"qa/unit-{unit:02d}/solution_closure.json" for unit in range(11, 20)],
        *[
            f"qa/unit-19/cumulative-media/unit-{unit:02d}_media.json"
            for unit in range(11, 20)
        ],
    }
    for unit in range(11, 20):
        required.update(
    ...
    add(
        blockers,
        [row.get("unit") for row in unit_bindings] == [11, 12, 13, 14, 15, 16, 17, 18, 19],
        "unit binding order/closure mismatch",
    )
    for row in unit_bindings:
        unit = int(row.get("unit", 0))
        if unit not in (11, 12, 13, 14, 15, 16, 17, 18, 19):
            continue
        add(
            blockers,
    ...
    add(blockers, wrapper.get("workflow") == "o011-unit19-wrapper-derivation-v1", "wrong wrapper derivation workflow")
    add(
        blockers,
        wrapper.get("geometry") == {"paper": "A4", "margin": "22mm", "centered": True, "class_option": "oneside"},
        "wrapper geometry contract mismatch",
    )
    add(blockers, wrapper.get("extension_units") == [11, 12, 13, 14, 15, 16, 17, 18, 19], "wrapper unit extension mismatch")
    add(
        blockers,
...
def verify_media(
    ...
    rows = [row for unit in range(1, 20) for row in units.get(str(unit), {}).get("media", [])]
    ...
    add(blockers, set(units) == {str(unit) for unit in range(1, 20)}, "media configuration does not cover exactly Units 1-19")
    ...
    for unit, expected_names in NEW_MEDIA.items():
        rel = f"qa/unit-19/cumulative-media/unit-{unit:02d}_media.json"
...
def write_early_failure(output_path: Path, blockers: list[str]) -> int:
    ...
    output_path.write_text(
        json.dumps(
            {
                "schema_version": 1,
                "workflow": "o011-through-unit19-pdf-structural-accessibility-qa-v1",
                "verdict": "FAIL",
                "passed": False,
                "blockers": blockers,
            },
...
def main() -> int:
    ...
    add(blockers, page_count > PREVIOUS_PUBLIC_PAGES, f"Unit 19 cumulative PDF has only {page_count} pages")
