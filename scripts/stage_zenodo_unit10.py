#!/usr/bin/env python3
"""Stage the seven deterministic, reader-first files for the Unit 10 release."""
...
HTML_ROOT_REL = "output/html/unit-10"
HTML_MANIFEST_REL = f"{HTML_ROOT_REL}/manifest.json"
HTML_ENTRY_REL = f"{HTML_ROOT_REL}/index.html"
BUILD_QA_REL = "qa/unit-10/build.json"
PDF_STRUCTURAL_QA_REL = "qa/unit-10/pdf_structural_qa.json"
PDF_VISUAL_QA_REL = "qa/unit-10/PDF_VISUAL_QA.json"
HTML_READER_QA_REL = "qa/unit-10/HTML_READER_QA.json"
HTML_VISUAL_QA_REL = "qa/unit-10/HTML_VISUAL_QA.json"
BACKEND_MANIFEST_REL = "backend/MANIFEST.json"
BACKEND_QA_REL = "qa/unit-10/backend.json"
LICENSE_SOURCE_REL = "qa/unit-10/LICENSE_RELEASE_UNIT10.md"
NOTES_SOURCE_REL = "qa/unit-10/RELEASE_NOTES_20260823.md"
...
DEFAULT_OUTPUT_REL = "output/release-unit10"
DEFAULT_METADATA_REL = "qa/unit-10/ZENODO_METADATA_UNIT10.json"
DEFAULT_RECEIPT_REL = "qa/unit-10/RELEASE_PREPARATION_RECEIPT.json"
...
SCRIPT_PATHS = (
    ...
    "scripts/publish_zenodo_unit10_rdm.py",
    "scripts/stage_zenodo_unit10.py",
...
ESSENTIAL_QA_PATHS = (
    "qa/unit-10/AUTHORITY_PREFLIGHT.json",
    "qa/unit-10/AUTHORITY_PREFLIGHT_VERIFY.json",
    "qa/unit-10/HTML_READER_QA.json",
    "qa/unit-10/HTML_VISUAL_QA.json",
    "qa/unit-10/MEDIA_ALIAS_RECEIPT.json",
    "qa/unit-10/PDF_VISUAL_QA.json",
    "qa/unit-10/POST_CORRECTION_MATH_QA.json",
    "qa/unit-10/WRAPPER_COMPATIBILITY_RECEIPT.json",
    "qa/unit-10/backend.json",
    "qa/unit-10/build.json",
    "qa/unit-10/lecture10_translation.json",
    "qa/unit-10/pdf_structural_qa.json",
    "qa/unit-10/solution_closure.json",
    "qa/unit-10/worksheet10_exercise09_solution_translation.json",
    "qa/unit-10/worksheet10_exercise10_solution_translation.json",
    "qa/unit-10/worksheet10_exercise15_solution_translation.json",
    "qa/unit-10/worksheet10_exercise25_solution_translation.json",
    "qa/unit-10/worksheet10_translation.json",
...
def collect_gate(root: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if build.get("workflow") != "o011-through-unit10-pdf-build-v1" or build.get("deterministic_clean_cycles") is not True:
        raise RuntimeError("current Unit 10 PDF build receipt is not the deterministic settled workflow")
    ...
    extension = backend_manifest.get("units08_10_extension")
    ...
    extension_qa = backend_qa.get("units08_10_extension")
...
def scan_bytes(label: str, payload: bytes) -> None:
    ...
    scan_payload = b"\n".join(
        b""
        if b"re.compile" in line and any(marker in line.lower() for marker in (b"users", b"token", b"password", b"secret"))
        else line
...
def privacy_scan_files(files: Iterable[tuple[str, Path]]) -> dict[str, int]:
    ...
    return {"files_considered": raw_count, "text_files_scanned": text_count, "private_locator_hits": 0, "credential_like_content_hits": 0}
...
def copy_source_tree(root: Path, staging: Path, documents: dict[str, Any], gate: dict[str, Any]) -> dict[str, Any]:
    ...
    add_mapping(mapping, "README.md", project_path(root, "qa/unit-10/PACKAGE_README.md"))
    ...
    for relative in (*AUTHORITY_LEDGER_PATHS, *SOURCE_SUPPORT_PATHS, *SCRIPT_PATHS, *BACKEND_PATHS, *ESSENTIAL_QA_PATHS):
    ...
    package_manifest = {
        "schema_version": 1,
        "workflow": "o011-unit10-compact-source-package-v1",
        "status": "active_partial",
        "coverage": "Kuliah 1–10 dan Lembar Kerja 1–10 dari 29 pasangan inti",
        ...
        "deliberate_exclusions": [
            "primary PDF and semantic HTML reader (published as separate reader-first files)",
...
def metadata_document() -> dict[str, Any]:
    description = (
        "Reader-first active_partial Indonesian adaptation through Kuliah 1–10 and Lembar Kerja 1–10. "
        "The release contains an A4 PDF, a reflowable semantic HTML reader, and a compact resumable "
        "source/backend/essential-QA package. Course text and the Indonesian adaptation are CC BY-SA 4.0; "
        "component media retain their recorded mixed public-domain, CC BY, and CC BY-SA rights. This is an "
    ...
    document = {
        "metadata": {
            "title": "Geometri Diferensial dan Manifold Mulus — Edisi Bahasa Indonesia (Batas Unit 10)",
...
def validate_metadata(document: dict[str, Any]) -> None:
    ...
    for required in ("active_partial", "Kuliah 1–10", "Lembar Kerja 1–10"):
...
def verify_public_surfaces(release: Path, license_source: Path) -> list[dict[str, Any]]:
    ...
    if (release / LICENSE_NAME).read_bytes() != license_source.read_bytes():
        raise RuntimeError("public LICENSE.md is not byte-identical to the approved Unit 10 license")
...
def atomic_write_new(path: Path, payload: bytes) -> None:
    ...
    temporary = path.with_name(path.name + ".tmp-unit10-stage")
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="unit10-release-stage-", dir=output_dir.parent) as temporary_name:
        ...
        receipt = {
            "schema_version": 1,
            "workflow": "o011-prepare-release-unit10-v1",
            "status": "pass",
