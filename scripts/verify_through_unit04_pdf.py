#!/usr/bin/env python3
"""Strict PDF QA for the cumulative Indonesian reader through Unit 4.
...
EXPECTED_METADATA = {
    ...
    "/Title": "Geometri Diferensial dan Manifold Mulus Pembaca kumulatif hingga Unit 4",
    "/Creator": "LaTeX with hyperref",
}

REQUIRED_TEXT_PATTERNS = {
    "work title": r"\bGeometri Diferensial dan Manifold Mulus\b",
    "cumulative boundary title": r"\bPembaca kumulatif hingga Unit 4\b",
...
REQUIRED_MINIMUM_COUNTS = {
    "supplied-solutions heading": (r"\bSolusi yang disediakan oleh sumber\b", 4),
...
REQUIRED_BOOKMARK_PATTERNS = {
    "Unit 4 part bookmark": r"^Unit 4$",
    "Unit 4 lecture bookmark": r"^Kuliah 4$",
    "Unit 4 worksheet bookmark": r"^Lembar Kerja 4$",
    "media-rights bookmark": r"^Atribusi dan Hak Media$",
}

REQUIRED_MEDIA_TEXT = {
    # Cumulative media closure through Unit 3; Unit 4 adds no media.
    "3d-function-6.svg",
    "MartinThoma",
...
FORBIDDEN_EXTRACTED_TEXT = {
    ...
    "German Unit 3/4 terminology residue": re.compile(
        ...
        r"Eigenwert(?:e|en)?|Eigenvektor(?:en)?|Hauptkrümmung(?:en)?)\b"
    ),
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Exact cumulative Unit 4 PDF path, relative to project root or absolute.",
    )
    parser.add_argument(
        "--expected-sha256",
        type=expected_hash,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("qa/unit-04/pdf_structural_qa.json"),
    )
    ...
    milestone_pages = {
        ...
        for label, pattern in ORDERED_UNIT4_MILESTONES
    }
    missing_milestones = [label for label, pages in milestone_pages.items() if not pages]
    if missing_milestones:
        blockers.append(f"missing Unit 4 page milestones: {missing_milestones}")
    else:
        ordered_pages: list[int] = []
        previous_page = 0
        for label, _ in ORDERED_UNIT4_MILESTONES:
            eligible = [page for page in milestone_pages[label] if page > previous_page]
            if not eligible:
                blockers.append(
                    f"Unit 4 milestone has no occurrence after the preceding milestone: {label}"
                )
                break
            previous_page = eligible[0]
            ordered_pages.append(previous_page)
        if len(ordered_pages) == len(ORDERED_UNIT4_MILESTONES) and ordered_pages != sorted(ordered_pages):
            blockers.append(
                f"Unit 4 milestones are out of reader order: {dict(zip((label for label, _ in ORDERED_UNIT4_MILESTONES), ordered_pages))}"
            )
    ...
    receipt = {
        "schema_version": 1,
        "workflow": "o011-through-unit04-pdf-structural-qa-v1",
        "verdict": verdict,
        "passed": passed,
        "execution_binding": {
            "project_root": ".",
        ...
        "content_closure": {
            ...
            "unit4_milestone_pages": milestone_pages,
