#!/usr/bin/env python3
"""Independent repeat-export verifier for the O011 Units 14--16 backend."""
...
import export_backend_v16 as exporter  # noqa: E402


VERIFY_WORKFLOW = "o011-verify-backend-v16"
...
def validate_state(root: Path) -> dict[str, Any]:
    ...
    if manifest.get("workflow") != exporter.WORKFLOW:
        raise RuntimeError("backend manifest is not the v16 workflow")
    extension = manifest.get("units14_16_extension", {})
    ...
    if set(units_manifest) != {"14", "15", "16"}:
        raise RuntimeError("backend manifest does not contain exactly Units 14--16")
    states = {str(value.get("translation_state")) for value in units_manifest.values()}
    if len(states) != 1:
        raise RuntimeError("Units 14--16 manifest has inconsistent translation states")
    state = states.pop()
    ...
    if any(line != canonical_json(record) for line, record in zip(lines[exporter.BASELINE_RECORD_COUNT :], suffix)):
        raise RuntimeError("noncanonical Units 14--16 JSONL record")
    if [str(record["id"]) for record in suffix] != sorted(str(record["id"]) for record in suffix):
        raise RuntimeError("Units 14--16 suffix IDs are not sorted")
    ...
    if b"".join(lines[exporter.BASELINE_RECORD_COUNT :]) != reconstructed_jsonl:
        raise RuntimeError("live reconstruction differs from the stored Units 14--16 JSONL suffix")
    ...
    if extension.get("entity_counts") != bundle["counts"]:
        raise RuntimeError("frozen Units 14--16 entity census changed")
    if extension.get("exercise_count") != 55 or extension.get("source_supplied_solution_count") != 13:
        raise RuntimeError("Units 14--16 exercise/solution extension census changed")
    ...
    required_claims = (
        ...
        "json_schema_valid",
        "units14_16_authority_solution_media_closure_current",
        "units14_16_translation_receipts_current",
        "units14_16_correction_manifests_current",
        "units14_16_post_correction_math_qa_current",
        "units1_13_public_prefix_byte_identical",
        "cumulative_exercises_342",
        "cumulative_source_supplied_solutions_48",
    ...
    if manifest.get("generator") != binding(root / "scripts/export_backend_v16.py", root):
        raise RuntimeError("manifest exporter binding is stale")
    if manifest.get("verifier") != binding(root / "scripts/verify_backend_v16.py", root):
    ...
    for context in bundle["contexts"]:
        ...
        expected_qa_count = 7 + len(solution_indices) + (4 if unit == 16 else 0)
    ...
    if sum(int(context["preflight"]["structure"]["worksheet_exercise_count"]) for context in bundle["contexts"]) != 55:
        raise RuntimeError("Units 14--16 exercise census changed")
    if sum(len(context["solution_indices"]) for context in bundle["contexts"]) != 13:
        raise RuntimeError("Units 14--16 source-supplied-solution census changed")

    reader = bundle["reader"]
    reader_artifacts = {
        "o011-artifact-u16-html-entry": "html_entry",
        "o011-artifact-u16-html-manifest": "html_manifest",
        "o011-artifact-u16-html-qa": "html_qa",
        "o011-artifact-u16-html-browser-qa": "html_browser_qa",
        "o011-artifact-u16-pdf": "pdf",
        "o011-artifact-u16-pdf-structural-qa": "pdf_structural_qa",
        "o011-artifact-u16-pdf-visual-qa": "pdf_visual_qa",
    }
    ...
    for event_id, key in (
        ("o011-qa-unit16-html-reader", "html_qa"),
        ("o011-qa-unit16-html-browser", "html_browser_qa"),
        ("o011-qa-unit16-pdf-structural", "pdf_structural_qa"),
        ("o011-qa-unit16-pdf-visual", "pdf_visual_qa"),
    ):
...
def main() -> None:
    ...
    unit_states = {str(value["translation_state"]) for value in current_manifest["units14_16_extension"]["units"].values()}
    if len(unit_states) != 1:
        raise RuntimeError("cannot repeat export with inconsistent unit states")
    state = unit_states.pop()
    command = [sys.executable, str(root / "scripts/export_backend_v16.py"), "--root", str(root), "--checkpoint", checkpoint, "--translation-state", state]
    ...
    receipt = {
        ...
        "units14_16_extension": {"records": len(second["suffix"]), "entity_counts": second["counts"], "census": census, "model_identification": exporter.MODEL_IDENTIFICATION, "exercise_count": 55, "source_supplied_solution_count": 13, "html_reader_bound": True, "html_browser_runtime_bound": True, "pdf_reader_bound": True},
        ...
        "checks": {"immutable_public_units1_13_prefix": True, "canonical_jsonl": True, "csv_projection_exact": True, "combined_json_schema_valid": True, "all_ids_unique": True, "all_references_resolve": True, "live_input_bindings_current": True, "source_target_segment_hashes_current": True, "solution_hint_point_media_closure": True, "component_rights_current": True, "correction_manifest_target_bindings_current": True, "qa_receipts_current": True, "model_provenance_current": True, "html_pdf_reader_all_or_nothing": True, "html_browser_runtime_qa_current": True, "pdf_structural_qa_current": True, "pdf_visual_qa_current": True},
    }
    receipt_path = root / "qa/unit-16/backend.json"
