#!/usr/bin/env python3
"""Stage the seven deterministic, reader-first Unit 16 release files.

This script mutates no remote state.  It refuses to stage until the current
PDF, HTML, backend, and durable-control surfaces form one hash-bound Unit 16
...
PREDECESSOR_RECORD_ID = 22097422
CONCEPT_RECORD_ID = 22059977

PDF_REL = "output/pdf/geometri-diferensial-manifold-mulus-hingga-unit-16-id.pdf"
HTML_ROOT_REL = "output/html/unit-16"
HTML_ENTRY_REL = f"{HTML_ROOT_REL}/index.html"
HTML_MANIFEST_REL = f"{HTML_ROOT_REL}/manifest.json"
BUILD_QA_REL = "qa/unit-16/build.json"
PDF_STRUCTURAL_QA_REL = "qa/unit-16/pdf_structural_qa.json"
PDF_VISUAL_QA_REL = "qa/unit-16/PDF_VISUAL_QA.json"
HTML_READER_QA_REL = "qa/unit-16/HTML_READER_QA.json"
HTML_BROWSER_QA_REL = "qa/unit-16/HTML_BROWSER_QA.json"
BACKEND_MANIFEST_REL = "backend/MANIFEST.json"
BACKEND_QA_REL = "qa/unit-16/backend.json"
LICENSE_SOURCE_REL = "qa/unit-16/LICENSE_RELEASE_UNIT16.md"
README_SOURCE_REL = "qa/unit-16/PACKAGE_README.md"
NOTES_SOURCE_REL = "qa/unit-16/RELEASE_NOTES_20260826.md"
METADATA_REL = "qa/unit-16/ZENODO_METADATA_UNIT16.json"
...
NOTES_NAME = "RELEASE_NOTES_UNIT16_20260826.md"
...
DEFAULT_OUTPUT_REL = "output/release-unit16"
DEFAULT_RECEIPT_REL = "qa/unit-16/RELEASE_PREPARATION_RECEIPT.json"
...
FIXED_SUPPORT_PATHS = (
    ...
    "build/generated/through-unit-16-driver.tex",
)

HTML_GENERATION_BINDING_PATHS = (
    *(f"qa/unit-{unit:02d}_media.json" for unit in range(1, 17)),
...
ESSENTIAL_QA_PATHS = (
    ...
    "qa/unit-16/AUTHORITY_PREFLIGHT.json",
    "qa/unit-16/AUTHORITY_PREFLIGHT_VERIFY.json",
    ...
    "qa/unit-16/POST_CORRECTION_MATH_QA.json",
    ...
    "qa/unit-16/solution_closure.json",
    "qa/unit-16/ZENODO_METADATA_UNIT16.json",
)
...
def verify_durable_controls(root: Path, gate: dict[str, Any]) -> dict[str, Any]:
    ...
    if missing:
        raise RuntimeError(f"durable controls do not yet bind the finalized Unit 16 checkpoint: {missing}")
    lowered = combined.lower()
    if "unit 17" not in lowered and "unit_17" not in lowered and "u17" not in lowered:
        raise RuntimeError("durable controls do not yet record Unit 17 as the next production action")
    return {
        ...
        "unit17_next_action_recorded": True,
    }
...
def collect_gate(root: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if build.get("workflow") != "o011-through-unit16-pdf-build-v1" or build.get("deterministic_clean_cycles") is not True:
        raise RuntimeError("Unit 16 PDF build receipt is not the deterministic settled workflow")
    ...
    if html_manifest.get("workflow") != "o011-export-html-v16" or html_manifest.get("model_identification") != MODEL:
    ...
    if html_qa.get("counts", {}).get("exercises") != 342 or html_qa.get("counts", {}).get("source_supplied_solutions") != 48:
    ...
    if backend_manifest.get("workflow") != "o011-export-backend-v16":
    ...
    if not isinstance(combined, dict) or combined.get("record_count") != 3208:
        raise RuntimeError("backend cumulative record count is not 3,208")
    entity_counts = combined.get("entity_counts")
    if not isinstance(entity_counts, dict) or entity_counts.get("correction") != 193:
        raise RuntimeError("backend cumulative correction count is not 193")
    closure = backend_manifest.get("reader_closure")
    if not isinstance(closure, dict) or closure.get("status") != "cumulative_html_pdf_reader_bound" or closure.get("through_unit") != 16:
        raise RuntimeError("backend manifest does not bind cumulative Unit 16 readers")
    ...
    if backend_qa.get("combined_records") != 3208 or backend_qa.get("determinism", {}).get("second_export_matches_first") is not True:
    ...
    if current["pdf"]["bytes"] != 7_241_359 or structural.get("pdf", {}).get("pages") != 261:
        raise RuntimeError("canonical PDF identity/page count changed")
    gate = {
        **current,
        "backend_records_csv": backend_outputs["records_csv"],
        "backend_records_jsonl": backend_outputs["records_jsonl"],
        "html_tree": html_tree,
        "coverage": {"units": 16, "exercises": 342, "source_supplied_solutions": 48, "backend_records": 3208},
    }
...
def copy_source_tree(root: Path, staging: Path, documents: dict[str, Any], gate: dict[str, Any]) -> dict[str, Any]:
    ...
    package_manifest = {
        "schema_version": 1,
        "workflow": "o011-unit16-compact-source-package-v1",
        "status": "active_partial",
        "coverage": "Kuliah 1–16 dan Lembar Kerja 1–16 dari 29 pasangan inti",
        ...
        "rebuild_commands": [
            ...
            "python scripts/test_html_v16_pipeline.py",
        ...
        "deliberate_exclusions": [
            "Unit 16 PDF and semantic HTML reader, published separately as the first two files",
...
def write_public_manifest(release: Path) -> None:
    ...
    document = {
        "schema_version": 1,
        "workflow": "o011-unit16-public-file-manifest-v1",
        "status": "active_partial",
        "coverage": "Kuliah 1–16 dan Lembar Kerja 1–16 dari 29 pasangan inti",
        "model_identification": MODEL,
...
def verify_public_surfaces(release: Path, license_source: Path) -> list[dict[str, Any]]:
    ...
    if (release / LICENSE_NAME).read_bytes() != license_source.read_bytes():
        raise RuntimeError("public LICENSE.md is not byte-identical to approved Unit 16 license")
...
def atomic_write_new(path: Path, payload: bytes) -> None:
    ...
    temporary = path.with_name(path.name + ".tmp-unit16-stage")
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="unit16-release-stage-", dir=output_dir.parent) as temporary_name:
        ...
        receipt = {
            "schema_version": 1,
            "workflow": "o011-prepare-release-unit16-v1",
            "status": "pass",
