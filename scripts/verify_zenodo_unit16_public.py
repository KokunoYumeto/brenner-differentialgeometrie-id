#!/usr/bin/env python3
"""Independently and anonymously verify the public Zenodo Unit 16 record."""
...
def anonymous_get(url: str, *, accept: str | None = None) -> urllib.response.addinfourl:
    ...
    headers = {"User-Agent": "O011-unit16-independent-readback/1.0"}
    if accept:
...
def validate_metadata(record: dict[str, Any]) -> None:
    ...
    if (
        ...
        or "Kuliah 1–16" not in description
        or "Lembar Kerja 1–16" not in description
        ...
        fail("public Unit 16 metadata lost its exact scope, rights, or provenance disclosure")
...
def verify_predecessor() -> dict[str, Any]:
    ...
    if (
        ...
        fail("public predecessor is not the exact Unit 13-r1 version")
    order, entries, preview = inventory(predecessor)
    if not entries:
        fail("public predecessor inventory is empty")
    return {
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path("qa/unit-16/SOURCE_PACKAGE_INTEGRITY.json"),
    )
    ...
    if {key: expected[PDF_NAME].get(key) for key in ("bytes", "sha256")} != PDF_IDENTITY:
        fail("release preparation changed validated Unit 16 PDF bytes")
    ...
    if (
        ...
        or publication.get("workflow") != "o011-publish-zenodo-unit16-v1"
        ...
        fail("publication receipt is not the exact passing Unit 16 transaction")
    ...
    if (
        ...
        fail("public Unit 16 lineage, access, preview, or inventory is not exact")
    validate_metadata(record)

    verified: list[dict[str, Any]] = []
    predecessor = verify_predecessor()
    with tempfile.TemporaryDirectory(prefix="o011-unit16-anonymous-readback-") as temporary:
                ...
                try:
                    with zipfile.ZipFile(destination, "r") as archive:
                        if archive.testzip() is not None or not archive.infolist():
                            fail("public Unit 16 source ZIP failed CRC/inventory verification")
                except zipfile.BadZipFile:
                    fail("public Unit 16 source ZIP is malformed")
            verified.append({"name": name, **actual})

    receipt = {
        "schema_version": 1,
        "workflow": "o011-independent-zenodo-unit16-public-readback-v1",
        "status": "pass",
