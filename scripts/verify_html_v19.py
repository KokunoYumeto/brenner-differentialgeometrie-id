#!/usr/bin/env python3
"""Strictly verify the cumulative O011 semantic HTML reader through Unit 19."""
...
def verify_animated_surface(
    ...
    expected_names = {"Fiddler crab mobius strip.gif", "Poincarehalfplaneconform.gif"}
    require(len(animated) == 2, "embedded animation occurrence count is not exactly two", errors)
    require({str(item.get("filename")) for item in animated} == expected_names, "embedded animation closure differs", errors)
    for declared in animated:
        ...
        require('data-animation-state="stopped"' in fragment, f"embedded animation does not start stopped: {figure_id}", errors)
        require(len(parser.images) == 1, f"embedded animation does not have exactly one image: {figure_id}", errors)
        ...
        require(unquote(image.get("src", "")) == f"assets/media/{expected_static}", f"embedded animation initial source is not its static frame: {expected_canonical}", errors)
        require(image.get("data-static-src") == image.get("src"), f"embedded animation static source differs from its initial source: {expected_canonical}", errors)
        require(unquote(image.get("data-animated-src", "")) == f"assets/media/{expected_canonical}", f"embedded animation Play source is not the canonical GIF: {expected_canonical}", errors)
        require(bool(image.get("alt", "").strip()), f"embedded animation has empty alternative text: {expected_canonical}", errors)
        require(image.get("aria-describedby", "") in parser.ids, f"embedded animation description target is unresolved: {expected_canonical}", errors)

        buttons = {item["attrs"].get("data-animation-action"): item for item in parser.buttons}
        require(set(buttons) == {"play", "stop"}, f"native Play/Stop controls are not both present: {expected_canonical}", errors)
        for action, label in (("play", "Putar animasi"), ("stop", "Hentikan animasi")):
            button = buttons.get(action, {"attrs": {}, "text": ""})
            require(button["attrs"].get("type") == "button", f"{action} control is not a native button: {expected_canonical}", errors)
            require(button["attrs"].get("aria-controls") == image.get("id"), f"{action} control does not target the image: {expected_canonical}", errors)
            require(button["text"] == label, f"{action} control label differs: {expected_canonical}", errors)
            require("tabindex" not in button["attrs"], f"{action} control overrides native keyboard focus: {expected_canonical}", errors)
        require("disabled" not in buttons.get("play", {}).get("attrs", {}), f"Play is initially disabled: {expected_canonical}", errors)
        require("disabled" in buttons.get("stop", {}).get("attrs", {}), f"Stop is not initially disabled: {expected_canonical}", errors)

        downloads = [item for item in parser.links if "animated-media-download" in item["attrs"].get("class", "").split()]
        require(len(downloads) == 1, f"canonical embedded GIF download is absent or duplicated: {expected_canonical}", errors)
        if downloads:
            require(unquote(downloads[0]["attrs"].get("href", "")) == f"assets/media/{expected_canonical}", f"embedded GIF download target differs: {expected_canonical}", errors)
            require(downloads[0]["attrs"].get("download") == expected_canonical, f"embedded GIF download filename differs: {expected_canonical}", errors)
        require(len(parser.statuses) == 1, f"polite animation status is absent or duplicated: {expected_canonical}", errors)
        if parser.statuses:
            require(parser.statuses[0]["attrs"].get("aria-live") == "polite", f"animation status is not polite: {expected_canonical}", errors)
    ...
    for declared in animated:
        require((output / "assets/media" / str(declared.get("static_filename"))).is_file(), f"verified static animation frame is missing: {declared.get('filename')}", errors)
        require((output / "assets/media" / str(declared.get("filename"))).is_file(), f"canonical embedded GIF is missing: {declared.get('filename')}", errors)
...
def verify(root: Path, output: Path, restage: bool = True) -> dict[str, Any]:
    ...
    require(manifest.get("units") == list(range(1, 20)), "manifest unit sequence is not exactly 1--19", errors)
    ...
    expected_baseline = exporter.unit13_baseline(root)
    require(manifest.get("unit13_baseline") == expected_baseline, "exact public Unit 13 baseline binding changed", errors)
    expected_qa = exporter.load_live_qa_bindings(root)
    require(manifest.get("unit14_19_live_qa") == expected_qa, "live Unit 14--19 QA/translation bindings changed", errors)
    ...
    require('new ResizeObserver' in document and 'observer.observe(document.body)' in document, "deep-link stabilizer does not follow long MathJax-driven layout reflow", errors)
    ...
    require(len(expected_exercise_ids) == 394, "cumulative exercise census is not exactly 394", errors)
    require(len(expected_solution_map) == 54, "cumulative source-supplied-solution census is not exactly 54", errors)
    extension_census = {
        "14": {"exercises": 18, "solutions": [5, 6, 9, 11, 12, 13, 14]},
        "15": {"exercises": 16, "solutions": [1, 11, 12, 13]},
        "16": {"exercises": 21, "solutions": [1, 12]},
        "17": {"exercises": 19, "solutions": [2, 4]},
        "18": {"exercises": 21, "solutions": [8, 11, 13, 14]},
        "19": {"exercises": 12, "solutions": []},
    }
    for tag, expected in extension_census.items():
        actual = topology.get(tag, {}) if isinstance(topology, dict) else {}
        require(actual.get("worksheet", {}).get("exercises") == expected["exercises"], f"Unit {tag} exact exercise census changed", errors)
        require(actual.get("source_supplied_solution_indices") == expected["solutions"], f"Unit {tag} exact supplied-solution census changed", errors)
    ...
    for unit, expected in ((14, 0), (15, 0), (16, 2), (17, 1), (18, 2), (19, 0)):
        require(sum(int(item.get("unit", -1)) == unit for item in figure_list) == expected, f"Unit {unit} exact media-occurrence census changed", errors)
    ...
    require(".math.display{max-width:100%;min-width:0;contain:inline-size}" in css, "display-math inline-size containment is absent", errors)
    require(".math.inline{display:inline-block;max-width:100%;overflow-x:auto" in css, "mobile inline-math scrolling surface is absent", errors)
    require(".math.inline{display:inline-block;max-width:100%;overflow-x:auto;overflow-y:hidden;vertical-align:middle;contain:inline-size}" not in css, "mobile inline math still has zero-width inline-size containment", errors)
    ...
    require("hingga Unit 19" in readme and "Putar animasi" in readme and "papan ketik" in readme, "README scope/animation accessibility disclosure missing", errors)
    ...
    if restage and not errors:
        ...
        with tempfile.TemporaryDirectory(prefix=".verify-html-v19-a-", dir=output.parent) as first_tmp, tempfile.TemporaryDirectory(prefix=".verify-html-v19-b-", dir=output.parent) as second_tmp:
    ...
    if errors:
        raise RuntimeError("HTML v19 verification failed:\n- " + "\n- ".join(dict.fromkeys(errors)))
    ...
    return {
        ...
        "workflow": WORKFLOW,
        "status": "pass",
        "scope": "Cumulative Indonesian semantic HTML reader through Unit 19",
        "entry": exporter.file_binding(entry_path, root),
        "manifest": exporter.file_binding(manifest_path, root),
        "checks": {
            "exact_published_unit13_baseline_unchanged": True,
            "live_unit14_19_post_qa_and_translation_receipts": True,
        ...
        "unit13_baseline": {
            ...
            "zenodo_public_readback": expected_baseline["zenodo_public_readback"],
            "github_public_readback": expected_baseline["github_public_readback"],
        },
        "unit14_19_live_qa": expected_qa,
...
def main() -> None:
    ...
    output = (args.output or (root / "output/html/unit-19")).resolve()
    receipt = (args.receipt or (root / "qa/unit-19/HTML_READER_QA.json")).resolve()
    try:
