#!/usr/bin/env python3
"""Strict PDF QA for the cumulative Indonesian reader through Unit 5.
...
EXPECTED_METADATA = {
    ...
    "/Title": "Geometri Diferensial dan Manifold Mulus Pembaca kumulatif hingga Unit 5",
    "/Creator": "LaTeX with hyperref",
}

REQUIRED_TEXT_PATTERNS = {
    "work title": r"\bGeometri Diferensial dan Manifold Mulus\b",
    "cumulative boundary title": r"\bPembaca kumulatif hingga Unit 5\b",
    ...
    "Unit 4 part": r"\bBagian IV Unit 4\b",
    "Unit 4 lecture": r"\bKuliah 4: Pemetaan Weingarten\b",
    "Unit 4 worksheet": r"\bLembar Kerja 4\b",
    "Unit 4 final exercise": r"\bSoal 4\.15\.\s*\(6 poin\)",
    "Unit 4 solution 7": r"\bSolusi Soal 4\.7(?!\d)",
    "Unit 4 solution 10": r"\bSolusi Soal 4\.10(?!\d)",
    "Unit 4 solution 7 correction note": (
        r"\bsolusi sumber memuat rumus turunan yang rusak dan berhenti sebelum "
        r"menghitung kedua turunan berarah\b"
    ),
    "Unit 4 solution 10 correction note": (
        r"\bsolusi sumber menghitung diferensial medan normal tetapi tidak "
        r"menerapkan tanda minus dalam definisi pemetaan Weingarten\b"
    ),
    "Unit 4 tangent-symbol correction note": (
        r"\bsumber memakai huruf kecil p pada ruang tangen ini\b"
    ),
    "Unit 4 missing-normal correction note": (
        r"\bsumber kehilangan argumen N\s*\(?P\)? dalam hasil kali dalam\b"
    ),
    "Unit 4 graph-metric correction note": (
        r"\bsumber menghilangkan faktor invers metrik G\b"
    ),
    "Unit 4 cone-curve correction note": (
        r"\bkurva pada sumber tidak selalu melalui P dan tidak selalu tetap "
        r"berada pada kerucut\b"
    ),
    "Unit 4 C1-to-C2 correction note": (
        r"\bsumber hanya mengasumsikan h kelas C1\b"
    ),
    "Unit 5 part": r"\bBagian V Unit 5\b",
    "Unit 5 lecture": r"\bKuliah 5: Kelengkungan Utama\b",
    "Unit 5 worksheet": r"\bLembar Kerja 5\b",
    "Unit 5 final exercise": r"\bSoal 5\.15\.\s*\(2 poin\)",
    "Unit 5 solution 1": r"\bSolusi Soal 5\.1(?!\d)",
    "Unit 5 graph correction note": (
        r"\bsumber menunjuk contoh bola yang tidak terkait dan menghilangkan "
        r"faktor invers metrik G\b"
    ),
    "Unit 5 rotational correction note": (
        r"\bsumber hanya mengasumsikan keterdiferensialan meskipun memakai\s*f"
    ),
    "Unit 5 Euler correction note": (
        r"\bsumber hanya menyebut vektor-vektor utama\b"
    ),
    "Unit 5 orientation correction note": (
        r"\bsumber tidak menetapkan orientasi irisan bidang\b"
    ),
    "Unit 5 supplied-solution correction note": (
        r"\bsolusi sumber memakai reduksi ke matriks Hesse tanpa menyebut "
        r"matriks metrik grafik\b"
    ),
    "Unit 5 media introduction": r"\bSatu berkas berikut digunakan dalam Unit 5\b",
    "Unit 5 localized figure caption": (
        r"\bBidang-bidang kelengkungan utama, vektor normal, dan bidang tangen "
        r"pada sebuah permukaan minimal\b"
    ),
...
REQUIRED_MINIMUM_COUNTS = {
    "supplied-solutions heading": (r"\bSolusi yang disediakan oleh sumber\b", 5),
    "Unit 4 C1-to-C2 correction notes": (
        r"\bsumber hanya mengasumsikan h kelas C1\b",
        2,
    ),
}

EXPECTED_UNIT4_SOLUTION_HEADINGS = Counter({"7": 1, "10": 1})
UNIT4_SOLUTION_HEADING_RE = re.compile(r"\bSolusi Soal 4\.(\d+)(?!\d)")
EXPECTED_UNIT5_SOLUTION_HEADINGS = Counter({"1": 1})
UNIT5_SOLUTION_HEADING_RE = re.compile(r"\bSolusi Soal 5\.(\d+)(?!\d)")
EXPECTED_UNIT5_EXERCISE_HEADINGS = Counter({str(index): 1 for index in range(1, 16)})
UNIT5_EXERCISE_HEADING_RE = re.compile(r"(?<!Solusi )\bSoal 5\.(\d+)\.(?!\d)")
REQUIRED_UNIT5_GRADED_MARKERS = {
    "5.11": r"\bSoal 5\.11\.\s*\(4 poin\)",
    "5.12": r"\bSoal 5\.12\.\s*\(4 poin\)",
    "5.13": r"\bSoal 5\.13\.\s*\(6 poin\)",
    "5.14": r"\bSoal 5\.14\.\s*\(6 \(2\+2\+2\) poin\)",
    "5.15": r"\bSoal 5\.15\.\s*\(2 poin\)",
}
UNIT5_HINT_PATTERN = (
    r"\bPetunjuk: Selesaikan persamaan terhadap suatu variabel yang sesuai dan "
    r"perlakukan hipermuka tersebut sebagai grafik dalam variabel-variabel lainnya\."
)

ORDERED_UNIT5_MILESTONES = [
    ("Unit 5 part", r"\bBagian V Unit 5\b"),
    ("Unit 5 lecture", r"\bKuliah 5: Kelengkungan Utama\b"),
    ("Unit 5 worksheet", r"\bLembar Kerja 5\b"),
    ("Unit 5 solution 1", r"\bSolusi Soal 5\.1(?!\d)"),
...
REQUIRED_BOOKMARK_PATTERNS = {
    "Unit 5 part bookmark": r"^V Unit 5$",
    "Unit 5 lecture bookmark": r"^Kuliah 5$",
    "Unit 5 worksheet bookmark": r"^Lembar Kerja 5$",
    "media-rights bookmark": r"^Atribusi dan Hak Media$",
}

REQUIRED_MEDIA_TEXT = {
    # Cumulative media closure through Unit 5; Unit 4 adds no media.
    "3d-function-6.svg",
    "MartinThoma",
    ...
    "RokerHRO",
    "domain publik",
    # Unit 3 exact component closure retained in the Unit 4 reader.
    "Parabola circle.svg",
    "IkamusumeFan",
    "Euler spiral.svg",
    "AdiJapan",
    ...
    "Eric Gaba",
    "Sting",
    "berdasarkan sebuah gambar dalam buku",
}

CUMULATIVE_MEDIA_SURFACES = {
    "Parabola circle.svg": {
    ...
    "Minimal surface curvature planes-de.svg": {
        "creator": "Eric Gaba",
        "license": "CC BY-SA 3.0",
        "source_uri": "https://commons.wikimedia.org/wiki/File:Minimal_surface_curvature_planes-de.svg",
        "license_uri": "https://creativecommons.org/licenses/by-sa/3.0/",
    },
}
...
# two BY-SA 4.0 components and one BY-SA 3.0 component; Unit 4 adds no media;
# Unit 5 adds one BY-SA 3.0 component.
REQUIRED_MINIMUM_URI_COUNTS = Counter(
    {
        ...
        "https://commons.wikimedia.org/wiki/File:Minimal_surface_curvature_planes-de.svg": 1,
    }
)
...
FORBIDDEN_EXTRACTED_TEXT = {
    ...
    "German Unit 3/4/5 terminology residue": re.compile(
        ...
        r"Beschleunigung|Umkehrfunktion|Aufgabe(?:n)?|Lösung(?:en)?|"
        r"Weingartenabbildung|Weingarten-Abbildung|Tangentialraum|"
        r"Eigenwert(?:e|en)?|Eigenvektor(?:en)?|Hauptkrümmung(?:en)?|"
        r"Normalkrümmung(?:en)?|Normalebene(?:n)?|Gaußkrümmung(?:en)?|"
        r"Rotationsfläche(?:n)?|Tangentialebene(?:n)?|Normalenvektor(?:en)?)\b"
    ),
...
def media_surface_checks(full_text: str) -> dict[str, dict[str, object]]:
    ...
    for filename, expected in CUMULATIVE_MEDIA_SURFACES.items():
        contexts: list[str] = []
        start = 0
        while True:
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Exact cumulative Unit 5 PDF path, relative to project root or absolute.",
    )
    parser.add_argument(
        "--expected-sha256",
        type=expected_hash,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("qa/unit-05/pdf_structural_qa.json"),
    )
    ...
    fonts_not_embedded = [
        key for key, details in fonts.items() if not details["embedded"]
    ]
    if not fonts:
    ...
    unit4_solution_heading_counts = Counter(
        UNIT4_SOLUTION_HEADING_RE.findall(normalized_full_text)
    )
    if unit4_solution_heading_counts != EXPECTED_UNIT4_SOLUTION_HEADINGS:
        blockers.append(
            "Unit 4 supplied-solution headings are not exactly 4.7 and 4.10 once each: "
            f"{dict(sorted(unit4_solution_heading_counts.items()))}"
        )
    unit5_solution_heading_counts = Counter(
        UNIT5_SOLUTION_HEADING_RE.findall(normalized_full_text)
    )
    if unit5_solution_heading_counts != EXPECTED_UNIT5_SOLUTION_HEADINGS:
        blockers.append(
            "Unit 5 supplied-solution headings are not exactly 5.1 once: "
            f"{dict(sorted(unit5_solution_heading_counts.items()))}"
        )
    unit5_exercise_heading_counts = Counter(
        UNIT5_EXERCISE_HEADING_RE.findall(normalized_full_text)
    )
    if unit5_exercise_heading_counts != EXPECTED_UNIT5_EXERCISE_HEADINGS:
        blockers.append(
            "Unit 5 exercise headings are not exactly 5.1 through 5.15 once each: "
            f"{dict(sorted(unit5_exercise_heading_counts.items()))}"
        )
    missing_unit5_graded_markers = [
        label
        for label, pattern in REQUIRED_UNIT5_GRADED_MARKERS.items()
        if not re.search(pattern, normalized_full_text)
    ]
    if missing_unit5_graded_markers:
        blockers.append(
            f"missing Unit 5 graded point markers: {missing_unit5_graded_markers}"
        )
    unit5_hint_count = len(re.findall(UNIT5_HINT_PATTERN, normalized_full_text))
    if unit5_hint_count != 1:
        blockers.append(f"Unit 5 hint count is not exactly one: {unit5_hint_count}")

    milestone_pages = {
        label: page_hits(normalized_page_text, pattern)
        for label, pattern in ORDERED_UNIT5_MILESTONES
    }
    missing_milestones = [label for label, pages in milestone_pages.items() if not pages]
    if missing_milestones:
        blockers.append(f"missing Unit 5 page milestones: {missing_milestones}")
    else:
        ordered_pages: list[int] = []
        previous_page = 0
        for label, _ in ORDERED_UNIT5_MILESTONES:
            eligible = [page for page in milestone_pages[label] if page > previous_page]
            if not eligible:
                blockers.append(
                    f"Unit 5 milestone has no occurrence after the preceding milestone: {label}"
                )
                break
            previous_page = eligible[0]
            ordered_pages.append(previous_page)
        if len(ordered_pages) == len(ORDERED_UNIT5_MILESTONES) and ordered_pages != sorted(ordered_pages):
            blockers.append(
                f"Unit 5 milestones are out of reader order: {dict(zip((label for label, _ in ORDERED_UNIT5_MILESTONES), ordered_pages))}"
            )

    cumulative_media_checks = media_surface_checks(full_text)
    failed_media_surfaces = [
        filename for filename, check in cumulative_media_checks.items() if not check["passed"]
    ]
    if failed_media_surfaces:
        blockers.append(
            f"incomplete cumulative media attribution surfaces: {failed_media_surfaces}"
        )
    ...
    cumulative_source_uri_counts = {
        filename: actual_uri_counts[details["source_uri"]]
        for filename, details in CUMULATIVE_MEDIA_SURFACES.items()
    }
    cumulative_license_uri_counts = {
    ...
    receipt = {
        "schema_version": 1,
        "workflow": "o011-through-unit05-pdf-structural-qa-v1",
        "verdict": verdict,
        "passed": passed,
        "execution_binding": {
            "project_root": ".",
        ...
        "content_closure": {
            ...
            "unit4_solution_heading_counts": dict(
                sorted(unit4_solution_heading_counts.items())
            ),
            "expected_unit4_solution_heading_counts": dict(
                sorted(EXPECTED_UNIT4_SOLUTION_HEADINGS.items())
            ),
            "unit5_solution_heading_counts": dict(
                sorted(unit5_solution_heading_counts.items())
            ),
            "expected_unit5_solution_heading_counts": dict(
                sorted(EXPECTED_UNIT5_SOLUTION_HEADINGS.items())
            ),
            "unit5_exercise_heading_counts": dict(
                sorted(unit5_exercise_heading_counts.items())
            ),
            "expected_unit5_exercise_heading_counts": dict(
                sorted(EXPECTED_UNIT5_EXERCISE_HEADINGS.items())
            ),
            "missing_unit5_graded_markers": missing_unit5_graded_markers,
            "unit5_hint_count": unit5_hint_count,
            "unit5_milestone_pages": milestone_pages,
            "missing_media_attribution_text": missing_media_text,
            "cumulative_media_surface_checks": cumulative_media_checks,
            "failed_cumulative_media_surfaces": failed_media_surfaces,
        },
        "links": {
            ...
            "cumulative_source_uri_counts": cumulative_source_uri_counts,
            "cumulative_license_uri_counts": cumulative_license_uri_counts,
