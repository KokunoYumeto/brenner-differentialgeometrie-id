...
$qaDir = Join-Path $laneRoot 'qa\unit-16'
$workDir = Join-Path $laneRoot 'tmp\pdfs\through-unit16-build'
$outputDir = Join-Path $laneRoot 'output\pdf'
$outputPdf = Join-Path $outputDir 'geometri-diferensial-manifold-mulus-hingga-unit-16-id.pdf'
...
# the complete Unit 11--16 live mathematics boundary is not yet admitted.
foreach ($unit in 11..16) {
...
# configuration. The live configuration now extends through Unit 16, so an
...
# before any Unit 11--16 binding begins. The v10 scripts themselves remain
...
if (
    ...
    throw 'Live Units 1-16 media configuration changed during read-only Unit 10 prefix verification'
}
$prefixTransactionReceipt = [ordered]@{
    schema_version = 1
    workflow = 'o011-unit16-unit10-prefix-preservation-v2'
...
# Rebuild the exact Unit 11--16 static-media receipts against the live Unit 16
...
# cumulative media configuration gained Unit 16 entries. The Unit 12 GIF
...
foreach ($spec in $unitSpecs) {
    $unit = [int]$spec.unit
    $unitText = '{0:D2}' -f $unit
    $receiptRel = "qa/unit-16/cumulative-media/unit-${unitText}_media.json"
    Invoke-CheckedPython @(
    ...
    if ($mediaReceipt.media_config_sha256 -ne $liveConfigIdentity.sha256) {
        throw "Unit $unit media receipt is not bound to the live Unit 16 config"
    }
...
$aliasReceipt = [ordered]@{
    schema_version = 1
    workflow = 'o011-unit16-cumulative-media-aliases-v1'
    aliases = @($aliasRows)
...
# Derive the Unit 16 reader driver from the exact frozen Unit 10 wrapper. This
...
$driverRel = 'build/generated/through-unit-16-driver.tex'
...
foreach ($replacement in @(
    @('Pembaca kumulatif hingga Unit 10', 'Pembaca kumulatif hingga Unit 16'),
    @('Kuliah 1--10 dan Lembar Kerja 1--10', 'Kuliah 1--16 dan Lembar Kerja 1--16')
)) {
...
if (
    ...
    throw 'Derived Unit 16 driver lost the centered A4/22mm geometry contract'
}
$driverIdentity = Get-Identity -Path $driverPath
$wrapperReceipt = [ordered]@{
    schema_version = 1
    workflow = 'o011-unit16-wrapper-derivation-v1'
    ...
    extension_units = @(11, 12, 13, 14, 15, 16)
...
# the complete live build graph against changes during the Unit 16 cycles.
...
Add-GuardCurrent -RelativePath 'source/unit_media.json' -Label 'untouched live Unit 16 media configuration' | Out-Null
...
Add-GuardCurrent -RelativePath 'scripts/build_through_unit16.ps1' -Label 'Unit 16 build script' | Out-Null
Add-GuardCurrent -RelativePath 'scripts/verify_through_unit16_pdf.py' -Label 'Unit 16 structural verifier' | Out-Null
...
Add-GuardCurrent -RelativePath (Get-RelativeProjectPath -Path $aliasReceiptPath) -Label 'Unit 16 media alias receipt' | Out-Null
Add-GuardCurrent -RelativePath (Get-RelativeProjectPath -Path $wrapperReceiptPath) -Label 'Unit 16 wrapper receipt' | Out-Null
Add-GuardExpected -RelativePath $driverRel -Bytes $driverIdentity.bytes -Sha256 $driverIdentity.sha256 -Label 'Unit 16 driver'

$auxiliaryNames = @(
    'through-unit-16.aux', 'through-unit-16.log', 'through-unit-16.out',
    'through-unit-16.pdf', 'through-unit-16.toc', 'through-unit-16.lof',
    'through-unit-16.fls', 'through-unit-16.fdb_latexmk'
)
...
function Invoke-BuildCycle {
    ...
    try {
        foreach ($pass in 1..3) {
            ...
            & $pdfLatex '-interaction=nonstopmode' '-halt-on-error' '-file-line-error' '-recorder' '-jobname=through-unit-16' 'generated/through-unit-16-driver.tex' 2>&1 |
    ...
    $built = Join-Path $buildDir 'through-unit-16.pdf'
...
try {
    ...
    $receipt = [ordered]@{
        schema_version = 1
        workflow = 'o011-through-unit16-pdf-build-v1'
        engine = $engineVersion
        command = 'pdflatex -interaction=nonstopmode -halt-on-error -file-line-error -recorder -jobname=through-unit-16 generated/through-unit-16-driver.tex (three passes per clean cycle; two clean cycles)'
        ...
        structural_verifier = [ordered]@{
            path = 'scripts/verify_through_unit16_pdf.py'
            bytes = $guardIdentities['scripts/verify_through_unit16_pdf.py'].bytes
            sha256 = $guardIdentities['scripts/verify_through_unit16_pdf.py'].sha256
            output = 'qa/unit-16/pdf_structural_qa.json'
        }
    }
    ...
    Invoke-CheckedPython @((Join-Path $laneRoot 'scripts\verify_through_unit16_pdf.py')) | Out-Null
    $structuralQa = Read-JsonFile -Path (Join-Path $qaDir 'pdf_structural_qa.json')
    if ($structuralQa.passed -ne $true) {
        throw 'Cumulative Unit 16 structural/accessibility QA did not pass'
    }
}
finally {
    if (Test-Path -LiteralPath $temporaryJpegAlias -PathType Leaf) {
