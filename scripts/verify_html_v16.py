#!/usr/bin/env python3
"""Strictly verify the cumulative O011 semantic HTML reader through Unit 16."""
...
import export_html_v16 as exporter
...
def verify(root: Path, output: Path, restage: bool = True) -> dict[str, Any]:
    ...
    require(manifest.get("units") == list(range(1, 17)), "manifest unit sequence is not exactly 1--16", errors)
    ...
    require(manifest.get("unit14_16_live_qa") == expected_qa, "live Unit 14--16 QA/translation bindings changed", errors)
    ...
    require(len(expected_exercise_ids) == 342, "cumulative exercise census is not exactly 342", errors)
    require(len(expected_solution_map) == 48, "cumulative source-supplied-solution census is not exactly 48", errors)
    extension_census = {
    ...
    for unit, expected in ((14, 0), (15, 0), (16, 2)):
    ...
    require(".math.inline{display:inline-block;max-width:100%;overflow-x:auto" in css, "mobile inline-math scrolling containment is absent", errors)
    ...
    require("hingga Unit 16" in readme and "Putar animasi" in readme and "papan ketik" in readme, "README scope/animation accessibility disclosure missing", errors)
    ...
    if restage and not errors:
        ...
        with tempfile.TemporaryDirectory(prefix=".verify-html-v16-a-", dir=output.parent) as first_tmp, tempfile.TemporaryDirectory(prefix=".verify-html-v16-b-", dir=output.parent) as second_tmp:
    ...
    if errors:
        raise RuntimeError("HTML v16 verification failed:\n- " + "\n- ".join(dict.fromkeys(errors)))
    ...
    return {
        ...
        "workflow": WORKFLOW,
        "status": "pass",
        "scope": "Cumulative Indonesian semantic HTML reader through Unit 16",
        ...
        "checks": {
            ...
            "live_unit14_16_post_qa_and_translation_receipts": True,
...
def main() -> None:
    ...
    receipt = (args.receipt or (root / "qa/unit-16/HTML_READER_QA.json")).resolve()
    try:
