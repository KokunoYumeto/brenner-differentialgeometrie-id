#!/usr/bin/env python3
"""Independently verify the cumulative Unit 16 source package.
...
identical to each other and to the canonical staged Unit 16 artifacts.
...
SCHEMA_VERSION = 1
WORKFLOW = "o011-verify-source-package-unit16-v1"
MODEL_IDENTIFICATION = "OpenAI Codex gpt-5.6-sol, Ultra"
ZIP_TIMESTAMP = (2026, 8, 26, 0, 0, 0)

DEFAULT_RELEASE_DIR = "output/release-unit16"
DEFAULT_SOURCE_ZIP = (
    "output/release-unit16/"
    "geometri-diferensial-manifold-mulus-unit16-source-20260826.zip"
)
DEFAULT_RECEIPT = "qa/unit-16/SOURCE_PACKAGE_INTEGRITY.json"
CANONICAL_PDF = (
    "output/pdf/geometri-diferensial-manifold-mulus-hingga-unit-16-id.pdf"
)
STAGED_PDF_NAME = "geometri-diferensial-manifold-mulus-hingga-unit-16-id.pdf"
STAGED_HTML_NAME = (
...
OUTER_NOTES = "RELEASE_NOTES_UNIT16_20260826.md"
CANONICAL_HTML_ROOT = "output/html/unit-16"
CANONICAL_BACKEND = (
...
EXPECTED_RECORD_COUNT = 3208
EXPECTED_CORRECTION_RECORD_COUNT = 193
EXPECTED_ADVERSE_ROWS = 193
EXPECTED_TERMINOLOGY_ROWS = 258
EXPECTED_ASSET_RECORD_COUNT = 25
EXPECTED_RIGHTS_RECORD_COUNT = 27
EXPECTED_MEDIA_RIGHTS_ROWS = 36
EXPECTED_EXERCISES = 342
EXPECTED_SOURCE_SOLUTIONS = 48
...
REQUIRED_BUILD_PATHS = (
    ...
    "scripts/test_html_v16_pipeline.py",
    ...
    "scripts/stage_zenodo_unit16.py",
...
REQUIRED_CURRENT_QA = (
    ...
    "qa/unit-16/HTML_BROWSER_QA.json",
...
def verify_backend_and_ledgers(root: Path) -> dict[str, Any]:
    ...
    if len(records) != EXPECTED_RECORD_COUNT:
        raise RuntimeError(
            f"backend record count differs: {len(records)} != {EXPECTED_RECORD_COUNT}"
        )
    ...
    if counts["correction"] != EXPECTED_CORRECTION_RECORD_COUNT:
        raise RuntimeError("backend correction-record count differs")
    if counts["asset"] != EXPECTED_ASSET_RECORD_COUNT:
        raise RuntimeError("backend asset-record count differs")
    if counts["rights"] != EXPECTED_RIGHTS_RECORD_COUNT:
        raise RuntimeError("backend rights-record count differs")
    ...
    if len(adverse) != EXPECTED_ADVERSE_ROWS:
        raise RuntimeError("adverse-ledger row count differs")
    if len(terminology) != EXPECTED_TERMINOLOGY_ROWS:
        raise RuntimeError("terminology-ledger row count differs")
    ...
    # Historic adverse IDs and later target-correction IDs have separate
    # namespaces, so some four-digit suffixes legitimately repeat.  The
    # backend must preserve the exact ledger suffix multiset and the latest
    # disclosed correction must still be 0193.
    if (
        len(adverse_numbers) != EXPECTED_ADVERSE_ROWS
        or len(correction_numbers) != EXPECTED_CORRECTION_RECORD_COUNT
        or Counter(adverse_numbers) != Counter(correction_numbers)
        or max(adverse_numbers, default=0) != EXPECTED_CORRECTION_RECORD_COUNT
    ):
    ...
    if not isinstance(manifest, dict):
        raise RuntimeError("backend manifest is not an object")
    combined = manifest.get("combined", {})
    if combined.get("record_count") != EXPECTED_RECORD_COUNT:
        raise RuntimeError("backend manifest combined record count differs")
    ...
    receipt = load_json(project_file(root, "qa/unit-16/backend.json"))
    if (
        not isinstance(receipt, dict)
        or receipt.get("status") != "pass"
        or receipt.get("combined_records") != EXPECTED_RECORD_COUNT
    ):
        raise RuntimeError("Unit 16 backend verification receipt is not passing/current")
...
def verify_required_closure(
    ...
    pdf = file_binding(project_file(root, CANONICAL_PDF), root) if (root / CANONICAL_PDF).is_file() else None
    # The source ZIP deliberately omits the cumulative reader itself.  The
    # packaged controls must nevertheless bind the finalized checkpoint hash.
    control_text = "\n".join(
        (
    ...
    canonical_pdf_sha = "58f98853ab8eeb1beb2aa4ade6bd3c746b62b4fa42c3c692a03c17076cdb06b8"
    ...
    if missing_tokens:
        raise RuntimeError(
            "durable controls do not yet record the finalized Unit 16 checkpoint: "
            + ", ".join(missing_tokens)
        )

    build = load_json(project_file(root, "qa/unit-16/build.json"))
    if not isinstance(build, dict) or build.get("workflow") != "o011-through-unit16-pdf-build-v1":
        raise RuntimeError("Unit 16 build receipt is missing or has the wrong workflow")
    build_inputs = validate_binding_collection(
        root, build.get("inputs"), "Unit 16 PDF transitive input", minimum=300
    )
    build_output = build.get("output", {})
    if (
        build_output.get("path") != CANONICAL_PDF
        or build_output.get("bytes") != 7_241_359
        or build_output.get("sha256") != canonical_pdf_sha
    ):
        raise RuntimeError("Unit 16 build receipt has a stale PDF identity")

    documented = "\n".join(flatten_strings(package_manifest)).lower()
    command_names = (
        "build_through_unit16.ps1",
        "export_html_v16.py",
        "verify_html_v16.py",
        "test_html_v16_pipeline.py",
        "verify_backend_v16.py",
    )
...
def rebuild_once(
    ...
    commands.append(
        run_command(
            [
                ...
                "Bypass",
                "-File",
                str(extracted_root / "scripts/build_through_unit16.ps1"),
            ],
            extracted_root,
    ...
    commands.append(
        run_command(
            [
                sys.executable,
                str(extracted_root / "scripts/export_html_v16.py"),
                "--root",
                str(extracted_root),
                "--output",
                str(html_output),
    ...
    commands.append(
        run_command(
            [
                sys.executable,
                str(extracted_root / "scripts/verify_html_v16.py"),
                "--root",
                str(extracted_root),
                "--output",
                str(html_output),
                "--receipt",
                str(extracted_root / "qa/unit-16/HTML_READER_QA.json"),
            ],
            extracted_root,
    ...
    commands.append(
        run_command(
            [sys.executable, str(extracted_root / "scripts/test_html_v16_pipeline.py")],
            extracted_root,
    ...
    commands.append(
        run_command(
            [
                sys.executable,
                str(extracted_root / "scripts/verify_backend_v16.py"),
                "--root",
                str(extracted_root),
            ],
            extracted_root,
    ...
    if html_inventory != canonical["html_inventory"]:
        raise RuntimeError("rebuilt HTML tree differs from the canonical Unit 16 tree")
    html_zip_check = verify_html_zip_against_inventory(staged_html_zip, html_inventory)
    replica_zip = extracted_root / "tmp/unit16-html-rebuilt.zip"
...
def clean_rebuild(
    ...
    temporary_root = Path(tempfile.mkdtemp(prefix=f"o011-u16-{cycle}-"))
...
def main() -> None:
    ...
    if not source_zip.is_file():
        raise RuntimeError(
            f"Unit 16 source ZIP is not staged yet at the expected path: {source_zip}"
        )
    if source_zip.name != STAGED_SOURCE_NAME:
        raise RuntimeError("Unit 16 source ZIP has the wrong filename")
    if not release_dir.is_dir() or source_zip.parent != release_dir:
        raise RuntimeError("Unit 16 source ZIP is not in the declared release directory")
    ...
    inspection_root = Path(tempfile.mkdtemp(prefix="o011-u16-inspect-"))
    ...
    checks = {
        ...
        "backend_3208_records_and_193_corrections": True,
        "adverse_193_and_terminology_258_rows": True,
