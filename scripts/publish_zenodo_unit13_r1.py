#!/usr/bin/env python3
"""Publish the corrective Unit 13 source-package revision to Zenodo.
...
source-package reconstruction receipt, the unchanged PDF/HTML reader bytes,
and public predecessor record 22096736.  It then resumes at most one
...
PREDECESSOR_VERSION = "2026.08.25-unit13"
...
RELEASE_DIR = Path("output/release-unit13-r1")
PREPARATION_WORKFLOW = "o011-prepare-release-unit13-source-r1-v1"
INTEGRITY_WORKFLOW = "o011-verify-source-package-unit13-r1-v1"
    ...
    try:
        with zipfile.ZipFile(archive_path, "r") as archive:
            if archive.testzip() is not None:
                fail("corrective source archive failed its CRC test")
            entries = safe_archive_entries(archive, "corrective source archive")
            if any(name.endswith("/00_control/PRIVATE_LOCAL_LOCATORS.md") or name == "00_control/PRIVATE_LOCAL_LOCATORS.md" for name in entries):
                fail("corrective source archive contains the private-locator control")

            anchor_suffix = "00_control/GOAL_AND_WORKFLOW.md"
            anchor = unique_suffix(entries, anchor_suffix, "corrective source archive")
            package_prefix = anchor[: -len(anchor_suffix)]

            control_root = root / "00_control"
            controls = [path for path in control_root.iterdir() if path.is_file() and path.name != "PRIVATE_LOCAL_LOCATORS.md"]
            if not controls:
                fail("local durable-control set is empty")
            for path in controls:
                member = package_prefix + f"00_control/{path.name}"
                if member not in entries:
                    fail(f"corrective source archive lacks durable control {path.name}")
                if archive_identity(archive, entries[member]) != {
                    "bytes": path.stat().st_size,
                    "sha256": digest(path),
                }:
                    fail(f"corrective source archive changed durable control {path.name}")
            ...
            for suffix in required:
                if suffix == "output/release-unit10":
                    package_directory = package_prefix + suffix + "/"
                    if not any(name.startswith(package_directory) for name in entries):
                        fail("corrective source archive lacks the Unit 10 release dependency closure")
                else:
                    member = package_prefix + suffix
                    if member not in entries:
                        fail(f"corrective source archive lacks required dependency {suffix}")
            ...
            if archive_license not in entries:
                fail("corrective source archive lacks its root LICENSE.md")
            if archive.read(entries[archive_license]) != staged_license:
                fail("source archive and staged LICENSE.md differ")
    except (OSError, RuntimeError, zipfile.BadZipFile):
        fail("unable to verify the corrective source archive")


def verify_integrity_receipt(path: Path, source: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        fail("source-package integrity receipt is not the passing Unit 13 r1 workflow")
    ...
    for item in clean:
        ...
        if not identity_occurs(item, PDF_IDENTITY) or not identity_occurs(item, HTML_IDENTITY):
            fail("a clean rebuild does not bind the unchanged PDF and HTML reader bytes")
...
def local_payload(
    ...
    if (
        ...
        fail("release-preparation receipt is not the passing local-only Unit 13 r1 workflow")
    ...
    if not isinstance(rows, list) or [row.get("filename") for row in rows if isinstance(row, dict)] != EXPECTED_ORDER:
        fail("release-preparation receipt does not declare the exact reader-first r1 inventory")

    release = root / RELEASE_DIR
    if not release.is_dir():
        fail("corrective release directory is absent")
    loose = sorted(path.name for path in release.iterdir() if path.is_file())
    if loose != sorted(EXPECTED_ORDER):
        fail("corrective release directory contains a missing or extra loose file")
    ...
    if total > MAX_PUBLIC_BYTES or stage.get("total_public_bytes") not in (None, total):
        fail("corrective public payload exceeds 500 MB or its bound total changed")
    if {key: local[PDF_NAME][key] for key in ("bytes", "sha256")} != PDF_IDENTITY:
        fail("corrective stage changed the validated Unit 13 PDF bytes")
    if {key: local[HTML_ZIP_NAME][key] for key in ("bytes", "sha256")} != HTML_IDENTITY:
        fail("corrective stage changed the validated Unit 13 HTML bytes")
    if not isinstance(stage.get("privacy_scan"), dict) or stage["privacy_scan"].get("status") != "pass":
        fail("corrective stage lacks a passing privacy scan")
    if not isinstance(stage.get("deterministic_archives"), dict) or stage["deterministic_archives"].get("status") != "pass":
        fail("corrective stage lacks passing deterministic-archive evidence")
...
def metadata_pair(path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if set(legacy) != required:
        fail("corrective metadata schema is not exact")
    if (
        legacy.get("title") != TITLE
        or legacy.get("version") != VERSION
        or legacy.get("publication_date") != PUBLICATION_DATE
    ):
        fail("corrective title, version, or publication date is not exact")
    ...
    if not isinstance(description, str) or any(text not in description for text in required_description):
        fail("corrective description lacks exact scope, rights, provenance, or correction disclosure")
    if description.count(MODEL) != 1 or PRIVATE_RE.search(description) or SECRET_RE.search(description):
        fail("corrective description duplicated the model identifier or exposed private material")
...
def exact_public(
    ...
    if projection(record) != projection({"metadata": modern}):
        return False
    try:
        _, entries, default_preview = public_inventory(record)
    except SystemExit:
        return False
    return (
        set(entries) == set(order)
...
def predecessor_ok(record: dict[str, Any]) -> None:
    ...
    if (
        ...
        or "Batas Unit 13" not in str(metadata.get("title"))
    ):
        fail("Zenodo predecessor is not the exact published Unit 13 record")
...
def ensure_no_duplicate_version(
    ...
    if allowed_exact_id is not None and len(matches) == 1 and record_id(matches[0]) == allowed_exact_id:
        return
    fail("duplicate or conflicting corrective Unit 13 version already exists")
...
def verify_unchanged_predecessor_readers(
    client: httpx.Client,
    predecessor: dict[str, Any],
    local: dict[str, dict[str, Any]],
) -> list[dict[str, Any]]:
    _, entries, _ = public_inventory(predecessor)
    results: list[dict[str, Any]] = []
    for name in (PDF_NAME, HTML_ZIP_NAME):
        if name not in entries:
            fail(f"predecessor lacks reader file {name}")
        if (
            entries[name].get("size") != local[name]["bytes"]
            or checksum_md5(entries[name]) != local[name]["md5"]
        ):
            fail(f"local corrective reader differs from predecessor inventory: {name}")
        actual = streamed_identity(client, content_url(entries[name]), f"predecessor {name}")
        wanted = {key: local[name][key] for key in ("bytes", "sha256", "md5")}
        if actual != wanted:
            fail(f"local corrective reader differs from anonymously read predecessor bytes: {name}")
        results.append({"name": name, **actual, "matches_predecessor": True})
    return results
...
def anonymous_readback(
    ...
    if not exact_public(record, modern, local, order):
        fail("public corrective Unit 13 record is not an exact metadata/file match")
...
def publication_receipt(
    ...
    record: dict[str, Any],
    legacy: dict[str, Any],
    reader_proof: list[dict[str, Any]],
    order: list[str],
    ...
    result: dict[str, Any] = {
        "schema_version": 1,
        "workflow": "o011-publish-zenodo-unit13-source-r1-v1",
        "status": "pass",
        ...
        "reader_content_unchanged": True,
        "reader_bytes_unchanged_from_predecessor": reader_proof,
...
def main() -> int:
    ...
    public_headers = {"User-Agent": "O011-unit13-r1-public-check/1.0", "Accept": API_MEDIA}
    with httpx.Client(trust_env=False, follow_redirects=True, timeout=300, headers=public_headers) as public:
        ...
        reader_proof = verify_unchanged_predecessor_readers(public, predecessor, local)
        ...
        headers = {
            ...
            "Accept": API_MEDIA,
            "User-Agent": "O011-unit13-r1-publisher/1.0",
        }
        ...
        try:
            ...
            if (
                ...
                # Zenodo clears ``metadata.version`` on a freshly inherited
                # new-version draft.  Accept that documented empty state only
                # after the exact concept/status/draft-ID checks above; the
                # approved r1 version is written immediately below.
                or unmodified_metadata.get("version") not in (None, PREDECESSOR_VERSION, VERSION)
                or "Batas Unit 13" not in str(unmodified_metadata.get("title"))
            ):
                fail("existing draft is not an inherited Unit 13 or exact r1 draft; refusing to mutate it")
            initial = {
                "metadata": modern,
            ...
            api_json(
                ...
                draft_url,
                (200,),
                "initial corrective metadata update",
                json=initial,
                headers={"Content-Type": "application/json", "Accept": API_MEDIA},
            )
            draft = api_json(auth, "GET", draft_url, (200,), "corrective draft read")
            if record_id(draft) != draft_id or concept_id(draft) != str(CONCEPT_ID):
                fail("corrective draft escaped the expected concept")
            ...
            api_json(
                ...
                draft_url,
                (200,),
                "final corrective metadata update",
                json=final,
                headers={"Content-Type": "application/json", "Accept": API_MEDIA},
            )
            verified = api_json(auth, "GET", draft_url, (200,), "final corrective draft verification")
            if projection({"metadata": verified.get("metadata") or {}}) != projection({"metadata": modern}):
                fail("final corrective draft metadata differs from the approved metadata")
            verified_entries = draft_files(verified)
            if set(verified_entries) != set(order) or len(verified_entries) != len(order):
                fail("final corrective draft file set differs from the exact staged inventory")
            for name in order:
                if (
                    verified_entries[name].get("size") != local[name]["bytes"]
                    or checksum_md5(verified_entries[name]) != local[name]["md5"]
                ):
                    fail(f"final corrective draft file identity mismatch: {name}")
            files_config = verified.get("files") or {}
            if files_config.get("default_preview") != PDF_NAME or files_config.get("order") not in ([], order):
                fail("final corrective draft lost reader-first preview/order semantics")

            latest_before = latest(public, predecessor)
            if record_id(latest_before) != PREDECESSOR_ID:
                fail("predecessor ceased to be latest before corrective publication")
            ensure_no_duplicate_version(public, PREDECESSOR_ID)
            published = api_json(
                auth,
                "POST",
                ...
                (201, 202),
                "corrective Unit 13 publication",
            )
        ...
        if recovered is None:
            fail("corrective publication did not become an exact anonymous metadata/byte match")
