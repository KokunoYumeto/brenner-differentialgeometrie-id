...
$qaDir = Join-Path $laneRoot 'qa\unit-13'
$workDir = Join-Path $laneRoot 'tmp\pdfs\through-unit13-build'
$outputDir = Join-Path $laneRoot 'output\pdf'
$outputPdf = Join-Path $outputDir 'geometri-diferensial-manifold-mulus-hingga-unit-13-id.pdf'
...
function Compare-IntegerSequence {
    ...
    $actualText = (@($Actual | ForEach-Object { [int]$_ }) -join ',')
...
# configuration. The live configuration now extends through Unit 13, so an
...
# before any Unit 11--13 binding begins. The v10 scripts themselves remain
...
if (
    ...
    throw 'Live Units 1-13 media configuration changed during read-only Unit 10 prefix verification'
}
$prefixTransactionReceipt = [ordered]@{
    schema_version = 1
    workflow = 'o011-unit13-unit10-prefix-preservation-v2'
...
foreach ($spec in $unitSpecs) {
    ...
    Compare-IntegerSequence `
        -Actual @($post.source_closure.supplied_solution_indices) `
    ...
    $expectedTranslations = @(
        [pscustomobject]@{
            ...
            post_target = $post.targets.lecture
        ...
        [pscustomobject]@{
            ...
            post_target = $post.targets.worksheet
    ...
    foreach ($item in $expectedTranslations) {
        ...
        if (
            $item.post_target.translation_receipt -ne $item.receipt -or
    ...
    $correctionRows = @()
    foreach ($row in @($post.correction_manifests)) {
...
# Rebuild the exact Unit 11--13 static-media receipts against the restored live
# configuration. The Unit 12 GIF derivative is frame zero by construction in
# prepare_unit_media.py and is rechecked below against its passing animation QA.
$mediaBindings = @()
foreach ($spec in $unitSpecs) {
    $unit = [int]$spec.unit
    $unitText = '{0:D2}' -f $unit
    $receiptRel = "qa/unit-${unitText}_media.json"
    Invoke-CheckedPython @(
    ...
    if ($mediaReceipt.media_config_sha256 -ne $liveConfigIdentity.sha256) {
        throw "Unit $unit media receipt is not bound to the live Unit 13 config"
    }
...
$unit12MediaReceipt = @($mediaBindings | Where-Object { $_.unit -eq 12 })[0].receipt
if (
    $animatedQa.bindings.media_receipt.path -ne $unit12MediaReceipt.path -or
    [long]$animatedQa.bindings.media_receipt.bytes -ne [long]$unit12MediaReceipt.bytes -or
    $animatedQa.bindings.media_receipt.sha256 -ne $unit12MediaReceipt.sha256
) {
    throw 'Unit 12 animation QA is not bound to the rebuilt media receipt'
}
...
# rebuilt Unit 10 prefix. The Unit 10 JPEG alias remains transient; the Unit 11
# PNG alias is a generated build artifact and is hash-receipted.
$persistentAliases = @(
    [ordered]@{
...
$aliasReceipt = [ordered]@{
    schema_version = 1
    workflow = 'o011-unit13-cumulative-media-aliases-v1'
    aliases = @($aliasRows)
...
# Derive the Unit 13 reader driver from the exact frozen Unit 10 wrapper. This
...
$driverRel = 'build/generated/through-unit-13-driver.tex'
...
foreach ($replacement in @(
    @('Pembaca kumulatif hingga Unit 10', 'Pembaca kumulatif hingga Unit 13'),
    @('Kuliah 1--10 dan Lembar Kerja 1--10', 'Kuliah 1--13 dan Lembar Kerja 1--13')
)) {
...
if (
    ...
    throw 'Derived Unit 13 driver lost the centered A4/22mm geometry contract'
}
$driverIdentity = Get-Identity -Path $driverPath
$wrapperReceipt = [ordered]@{
    schema_version = 1
    workflow = 'o011-unit13-wrapper-derivation-v1'
    ...
    extension_units = @(11, 12, 13)
    supplied_solutions = [ordered]@{ '11' = @(10, 14); '12' = @(11, 12); '13' = @(1, 10, 11, 16, 18, 19, 21, 22) }
...
# the complete live build graph against changes during the Unit 13 cycles.
...
Add-GuardCurrent -RelativePath 'source/unit_media.json' -Label 'untouched live Unit 13 media configuration' | Out-Null
...
Add-GuardCurrent -RelativePath 'scripts/build_through_unit13.ps1' -Label 'Unit 13 build script' | Out-Null
Add-GuardCurrent -RelativePath 'scripts/verify_through_unit13_pdf.py' -Label 'Unit 13 structural verifier' | Out-Null
...
Add-GuardCurrent -RelativePath (Get-RelativeProjectPath -Path $aliasReceiptPath) -Label 'Unit 13 media alias receipt' | Out-Null
Add-GuardCurrent -RelativePath (Get-RelativeProjectPath -Path $wrapperReceiptPath) -Label 'Unit 13 wrapper receipt' | Out-Null
Add-GuardExpected -RelativePath $driverRel -Bytes $driverIdentity.bytes -Sha256 $driverIdentity.sha256 -Label 'Unit 13 driver'

$auxiliaryNames = @(
    'through-unit-13.aux', 'through-unit-13.log', 'through-unit-13.out',
    'through-unit-13.pdf', 'through-unit-13.toc', 'through-unit-13.lof',
    'through-unit-13.fls', 'through-unit-13.fdb_latexmk'
)
...
function Invoke-BuildCycle {
    ...
    try {
        foreach ($pass in 1..3) {
            ...
            & $pdfLatex '-interaction=nonstopmode' '-halt-on-error' '-file-line-error' '-recorder' '-jobname=through-unit-13' 'generated/through-unit-13-driver.tex' 2>&1 |
    ...
    $built = Join-Path $buildDir 'through-unit-13.pdf'
...
try {
    ...
    $receipt = [ordered]@{
        schema_version = 1
        workflow = 'o011-through-unit13-pdf-build-v1'
        engine = $engineVersion
        command = 'pdflatex -interaction=nonstopmode -halt-on-error -file-line-error -recorder -jobname=through-unit-13 generated/through-unit-13-driver.tex (three passes per clean cycle; two clean cycles)'
        ...
        structural_verifier = [ordered]@{
            path = 'scripts/verify_through_unit13_pdf.py'
            bytes = $guardIdentities['scripts/verify_through_unit13_pdf.py'].bytes
            sha256 = $guardIdentities['scripts/verify_through_unit13_pdf.py'].sha256
            output = 'qa/unit-13/pdf_structural_qa.json'
        }
    }
    ...
    Invoke-CheckedPython @((Join-Path $laneRoot 'scripts\verify_through_unit13_pdf.py')) | Out-Null
    $structuralQa = Read-JsonFile -Path (Join-Path $qaDir 'pdf_structural_qa.json')
    if ($structuralQa.passed -ne $true) {
        throw 'Cumulative Unit 13 structural/accessibility QA did not pass'
    }
}
finally {
    if (Test-Path -LiteralPath $temporaryJpegAlias -PathType Leaf) {
