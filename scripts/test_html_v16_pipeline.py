#!/usr/bin/env python3
"""Bounded non-cumulative tests for the Unit 16 HTML pipeline.

The final Unit 15/16 POST QA receipts are intentionally allowed to be absent here.
...
def main() -> None:
    ...
    cumulative_output = root / "output/html/unit-16"
    ...
    check({int(path.parent.name[-2:]) for path in sources} == set(range(1, 17)), "source inventory does not span exactly Units 1--16")
    ...
    unit14_16_translations = {
        unit: exporter.unit_translation_bindings(root, unit) for unit in (14, 15, 16)
    }
    for unit, bindings in unit14_16_translations.items():
    ...
    with tempfile.TemporaryDirectory(prefix="o011-html-v16-bounded-") as temporary:
    ...
    final_posts = [root / f"qa/unit-{unit:02d}/POST_CORRECTION_MATH_QA.json" for unit in (14, 15, 16)]
    ...
    if final_gate_present:
        ...
        check(set(all_live) == {"14", "15", "16"}, "final live QA closure differs")
        contract = exporter.generation_contract(root)
        check(len(contract["generation_bindings"]) == 24, "generation media/rights binding count differs")
    else:
        try:
            ...
            raise RuntimeError("full live QA closure unexpectedly passed without all Unit 14--16 POST QA receipts")

    output_after = exporter.tree_inventory(cumulative_output) if cumulative_output.is_dir() else None
    check(output_after == output_before, "bounded test changed the cumulative Unit 16 output")
    result = {
        "status": "pass",
        ...
        "unit14_16_translation_receipts_bound": sum(len(value) for value in unit14_16_translations.values()),
        "unit14_16_final_post_qa_present": final_gate_present,
