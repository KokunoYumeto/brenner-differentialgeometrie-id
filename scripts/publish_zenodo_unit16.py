#!/usr/bin/env python3
"""Publish the verified Unit 16 checkpoint to the existing Zenodo concept.
...
and public predecessor record 22097422.  It then resumes at most one
...
PREDECESSOR_ID = 22097422
PREDECESSOR_DOI = "10.5281/zenodo.22097422"
PREDECESSOR_VERSION = "2026.08.25-unit13-r1"
VERSION = "2026.08.26-unit16"
PUBLICATION_DATE = "2026-08-26"
TITLE = "Geometri Diferensial dan Manifold Mulus — Edisi Bahasa Indonesia (Batas Unit 16)"
...
RELEASE_DIR = Path("output/release-unit16")
PREPARATION_WORKFLOW = "o011-prepare-release-unit16-v1"
INTEGRITY_WORKFLOW = "o011-verify-source-package-unit16-v1"
PDF_NAME = "geometri-diferensial-manifold-mulus-hingga-unit-16-id.pdf"
HTML_ZIP_NAME = "geometri-diferensial-manifold-mulus-unit16-html-20260826.zip"
SOURCE_ZIP_NAME = "geometri-diferensial-manifold-mulus-unit16-source-20260826.zip"
EXPECTED_ORDER = [
    PDF_NAME,
    HTML_ZIP_NAME,
    SOURCE_ZIP_NAME,
    ...
    try:
        with zipfile.ZipFile(archive_path, "r") as archive:
            if archive.testzip() is not None:
                fail("Unit 16 source archive failed its CRC test")
            entries = safe_archive_entries(archive, "Unit 16 source archive")
            if any(name.endswith("/00_control/PRIVATE_LOCAL_LOCATORS.md") or name == "00_control/PRIVATE_LOCAL_LOCATORS.md" for name in entries):
                fail("Unit 16 source archive contains the private-locator control")

            anchor_suffix = "00_control/GOAL_AND_WORKFLOW.md"
            anchor = unique_suffix(entries, anchor_suffix, "Unit 16 source archive")
            ...
            for path in controls:
                ...
                if member not in entries:
                    fail(f"Unit 16 source archive lacks durable control {path.name}")
                if archive_identity(archive, entries[member]) != {
                    "bytes": path.stat().st_size,
                    "sha256": digest(path),
                }:
                    fail(f"Unit 16 source archive changed durable control {path.name}")

            required = (
                "README.md",
                "LICENSE.md",
                "scripts/build_through_unit16.ps1",
                "scripts/export_html_v16.py",
                "scripts/export_backend_v16.py",
                "scripts/verify_through_unit16_pdf.py",
                "scripts/verify_html_v16.py",
                "scripts/verify_backend_v16.py",
                "scripts/verify_source_package_unit16.py",
                "backend/records.jsonl",
                "backend/records.csv",
                "backend/MANIFEST.json",
                "qa/unit-16/build.json",
                "qa/unit-16/pdf_structural_qa.json",
                "qa/unit-16/PDF_VISUAL_QA.json",
                "qa/unit-16/HTML_READER_QA.json",
                "qa/unit-16/backend.json",
            )
            for suffix in required:
                member = package_prefix + suffix
                if member not in entries:
                    fail(f"Unit 16 source archive lacks required dependency {suffix}")
...
def verify_integrity_receipt(
    ...
    if (
        ...
        fail("source-package integrity receipt is not the passing Unit 16 workflow")
    ...
    for item in clean:
        ...
        if not identity_occurs(item, PDF_IDENTITY) or not identity_occurs(item, html):
            fail("a clean rebuild does not bind the validated Unit 16 PDF and HTML reader bytes")
...
def local_payload(
    ...
    if (
        ...
        fail("release-preparation receipt is not the passing local-only Unit 16 workflow")
    ...
    if not isinstance(rows, list) or [row.get("filename") for row in rows if isinstance(row, dict)] != EXPECTED_ORDER:
        fail("release-preparation receipt does not declare the exact reader-first Unit 16 inventory")

    release = root / RELEASE_DIR
    if not release.is_dir():
        fail("Unit 16 release directory is absent")
    loose = sorted(path.name for path in release.iterdir() if path.is_file())
    if loose != sorted(EXPECTED_ORDER):
        fail("Unit 16 release directory contains a missing or extra loose file")
...
def metadata_pair(path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if set(legacy) != required:
        fail("Unit 16 metadata schema is not exact")
    if (
        legacy.get("title") != TITLE
        or legacy.get("version") != VERSION
        or legacy.get("publication_date") != PUBLICATION_DATE
    ):
        fail("Unit 16 title, version, or publication date is not exact")
    ...
    if not isinstance(description, str) or any(text not in description for text in required_description):
        fail("Unit 16 description lacks exact scope, rights, or provenance disclosure")
    if description.count(MODEL) != 1 or PRIVATE_RE.search(description) or SECRET_RE.search(description):
        fail("Unit 16 description duplicated the model identifier or exposed private material")
...
def predecessor_ok(record: dict[str, Any]) -> None:
    ...
    if (
        ...
        fail("Zenodo predecessor is not the exact published Unit 13-r1 boundary")
...
def ensure_no_duplicate_version(
    ...
    if allowed_exact_id is not None and len(matches) == 1 and record_id(matches[0]) == allowed_exact_id:
        return
    fail("duplicate or conflicting Unit 16 version already exists")
...
def verify_predecessor_boundary(predecessor: dict[str, Any]) -> dict[str, Any]:
    ...
    if not entries:
        fail("predecessor has no public file inventory")
    return {
...
def anonymous_readback(
    ...
    if not exact_public(record, modern, local, order):
        fail("public Unit 16 record is not an exact metadata/file match")
...
def publication_receipt(
    ...
    result: dict[str, Any] = {
        "schema_version": 1,
        "workflow": "o011-publish-zenodo-unit16-v1",
        "status": "pass",
        ...
        "version": VERSION,
        "coverage": "active_partial_through_unit_16",
...
def main() -> int:
    ...
    public_headers = {"User-Agent": "O011-unit16-public-check/1.0", "Accept": API_MEDIA}
    with httpx.Client(trust_env=False, follow_redirects=True, timeout=300, headers=public_headers) as public:
        ...
        predecessor_proof = verify_predecessor_boundary(predecessor)
        ...
        headers = {
            ...
            "Accept": API_MEDIA,
            "User-Agent": "O011-unit16-publisher/1.0",
        }
        ...
        try:
            ...
            if (
                ...
                # approved Unit 16 version is written immediately below.
                or unmodified_metadata.get("version") not in (None, PREDECESSOR_VERSION, VERSION)
                or not any(
                    marker in str(unmodified_metadata.get("title"))
                    for marker in ("Batas Unit 13", "Batas Unit 16")
                )
            ):
                fail("existing draft is not the inherited Unit 13-r1 or exact Unit 16 draft")
            initial = {
                "metadata": modern,
            ...
            api_json(
                ...
                draft_url,
                (200,),
                "initial Unit 16 metadata update",
                json=initial,
                headers={"Content-Type": "application/json", "Accept": API_MEDIA},
            )
            draft = api_json(auth, "GET", draft_url, (200,), "Unit 16 draft read")
            if record_id(draft) != draft_id or concept_id(draft) != str(CONCEPT_ID):
                fail("Unit 16 draft escaped the expected concept")
            ...
            api_json(
                ...
                draft_url,
                (200,),
                "final Unit 16 metadata update",
                json=final,
                headers={"Content-Type": "application/json", "Accept": API_MEDIA},
            )
            verified = api_json(auth, "GET", draft_url, (200,), "final Unit 16 draft verification")
            if projection({"metadata": verified.get("metadata") or {}}) != projection({"metadata": modern}):
                fail("final Unit 16 draft metadata differs from the approved metadata")
            verified_entries = draft_files(verified)
            if set(verified_entries) != set(order) or len(verified_entries) != len(order):
                fail("final Unit 16 draft file set differs from the exact staged inventory")
            for name in order:
                if (
                    verified_entries[name].get("size") != local[name]["bytes"]
                    or checksum_md5(verified_entries[name]) != local[name]["md5"]
                ):
                    fail(f"final Unit 16 draft file identity mismatch: {name}")
            ...
            if files_config.get("default_preview") != PDF_NAME or effective_order != order:
                fail("final Unit 16 draft lost reader-first preview/order semantics")

            latest_before = latest(public, predecessor)
            if record_id(latest_before) != PREDECESSOR_ID:
                fail("predecessor ceased to be latest before Unit 16 publication")
            ensure_no_duplicate_version(public, PREDECESSOR_ID)
            published = api_json(
                auth,
                "POST",
        ...
        if recovered is None:
            fail("Unit 16 publication did not become an exact anonymous metadata/byte match")
