#!/usr/bin/env python3
"""Publish the verified Unit 13 checkpoint in the existing Zenodo lineage.

This transaction is deliberately fail-closed.  It proves the exact local
seven-file payload and the public Unit 10 predecessor before reading a token,
...
PREDECESSOR_ID = 22073928
PREDECESSOR_DOI = "10.5281/zenodo.22073928"
VERSION = "2026.08.25-unit13"
PUBLICATION_DATE = "2026-08-25"
MODEL = "OpenAI Codex gpt-5.6-sol, Ultra"
API_MEDIA = "application/vnd.inveniordm.v1+json"
RELEASE_DIR = Path("output/release-unit13")
...
SOURCE_ZIP_NAME = "geometri-diferensial-manifold-mulus-brenner-id-unit13-source-20260825.zip"
LICENSE_SUPPORT = Path("qa/unit-13/LICENSE_RELEASE_UNIT13.md")
PACKAGE_README_SUPPORT = Path("qa/unit-13/PACKAGE_README.md")
BUILD_RECEIPT = Path("qa/unit-13/build.json")
HTML_RECEIPT = Path("qa/unit-13/HTML_READER_QA.json")
PDF_RECEIPT = Path("qa/unit-13/pdf_structural_qa.json")
EXPECTED_ORDER = [
    PDF_NAME,
    HTML_ZIP_NAME,
    SOURCE_ZIP_NAME,
    "LICENSE.md",
    "RELEASE_NOTES_20260825.md",
    "FILE_MANIFEST.csv",
    "CHECKSUMS.sha256",
]
...
def verify_release_support(root: Path, local: dict[str, dict[str, Any]]) -> None:
    try:
        ...
        fail("unable to read the approved Unit 13 release-support text")
    if local["LICENSE.md"]["path"].read_bytes() != approved_license:
        fail("staged LICENSE.md is not byte-identical to the approved Unit 13 rights text")
    ...
    if any(value not in license_text for value in required_license_text) or license_text.count(MODEL) != 1:
        fail("approved Unit 13 rights text lost its CC BY-SA 4.0, per-media-rights, or model disclosure")
    if MODEL not in readme_text or readme_text.count(MODEL) != 1:
        fail("approved Unit 13 package README lacks the exact single model identification")

    pdf_receipt = load_object(root / PDF_RECEIPT, "Unit 13 PDF receipt")
    ...
    if (
        ...
        or pdf_receipt.get("workflow") != "o011-through-unit13-pdf-structural-accessibility-qa-v1"
        ...
        fail("staged PDF is not the exact passing Unit 13 reader with model and per-media-rights closure")

    html_receipt = load_object(root / HTML_RECEIPT, "Unit 13 HTML receipt")
    html_checks = html_receipt.get("checks")
    html_inventory = html_receipt.get("output_inventory")
    if (
        html_receipt.get("schema_version") != 1
        or html_receipt.get("workflow") != "o011-verify-html-v13"
        or html_receipt.get("status") != "pass"
        or not isinstance(html_checks, dict)
        or html_checks.get("local_media_alt_caption_attribution_rights_closure") is not True
        ...
        fail("Unit 13 HTML rights/hash receipt is absent or not passing")
    try:
        with zipfile.ZipFile(local[HTML_ZIP_NAME]["path"], "r") as archive:
            entries = archive_entries(archive, "Unit 13 HTML archive")
            matched: set[str] = set()
            for item in html_inventory:
                if not isinstance(item, dict) or not isinstance(item.get("path"), str):
                    fail("Unit 13 HTML receipt inventory is malformed")
                prefix = "output/html/unit-13/"
                if not item["path"].startswith(prefix):
                    fail("Unit 13 HTML receipt escaped the reader root")
                member = unique_archive_suffix(entries, item["path"][len(prefix):], "Unit 13 HTML archive")
                if member in matched or archive_member_identity(archive, entries[member]) != {"bytes": item.get("bytes"), "sha256": item.get("sha256")}:
                    fail(f"Unit 13 HTML archive member differs from its passing receipt: {item['path']}")
                matched.add(member)
            if matched != set(entries):
                fail("Unit 13 HTML archive contains files outside the passing reader inventory")
    except (OSError, zipfile.BadZipFile, RuntimeError):
        fail("unable to verify the Unit 13 HTML archive")

    build_receipt = load_object(root / BUILD_RECEIPT, "Unit 13 build receipt")
    if build_receipt.get("schema_version") != 1 or build_receipt.get("workflow") != "o011-through-unit13-pdf-build-v1":
        fail("Unit 13 build receipt is not the expected workflow")
    rights_bindings = [
        receipt_file_binding(build_receipt, "authority/brenner_media_rights_manifest.csv", "Unit 13 build receipt"),
        receipt_file_binding(build_receipt, "source/unit_media.json", "Unit 13 build receipt"),
    ]
    try:
        with zipfile.ZipFile(local[SOURCE_ZIP_NAME]["path"], "r") as archive:
            entries = archive_entries(archive, "Unit 13 source archive")
            for binding in rights_bindings:
                member = unique_archive_suffix(entries, str(binding["path"]), "Unit 13 source archive")
                if archive_member_identity(archive, entries[member]) != {"bytes": binding["bytes"], "sha256": binding["sha256"]}:
                    fail(f"Unit 13 source archive changed its rights binding: {binding['path']}")
            license_member = unique_archive_suffix(entries, "LICENSE.md", "Unit 13 source archive")
            readme_member = unique_archive_suffix(entries, "README.md", "Unit 13 source archive")
            if archive.read(entries[license_member]) != approved_license or archive.read(entries[readme_member]) != approved_readme:
                fail("Unit 13 source archive changed its approved license or package README")
    except (OSError, zipfile.BadZipFile, RuntimeError):
        fail("unable to verify the Unit 13 source archive")
...
def local_payload(root: Path, receipt_path: Path) -> tuple[dict[str, Any], dict[str, dict[str, Any]], list[str]]:
    ...
    if stage.get("schema_version") != 1 or stage.get("workflow") != "o011-prepare-release-unit13-v1" or stage.get("status") != "pass":
        fail("release-preparation receipt is not the passing Unit 13 workflow")
...
def legacy_and_modern_metadata(path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if set(metadata) != required:
        fail("Unit 13 metadata schema is not exact")
    expected_title = "Geometri Diferensial dan Manifold Mulus — Edisi Bahasa Indonesia (Batas Unit 13)"
    if metadata.get("title") != expected_title or metadata.get("version") != VERSION or metadata.get("publication_date") != PUBLICATION_DATE:
        fail("Unit 13 title/version/publication date is not exact")
    ...
    if "Kuliah 1–13" not in description or "Lembar Kerja 1–13" not in description:
        fail("metadata description does not state exact Unit 1--13 coverage")
...
def predecessor_ok(record: dict[str, Any]) -> None:
    ...
    if record.get("id") != PREDECESSOR_ID or str(record.get("conceptrecid")) != CONCEPT_ID or record.get("doi") != PREDECESSOR_DOI or record.get("conceptdoi") != CONCEPT_DOI:
        fail("Zenodo predecessor is not the exact Unit 10 concept record")
    if metadata.get("version") != "2026.08.23-unit10" or "Batas Unit 10" not in str(metadata.get("title")):
        fail("Zenodo predecessor is not the exact published Unit 10 boundary")
...
def ensure_no_duplicate_public_version(client: httpx.Client, seed_id: int, allowed_exact_id: int | None = None) -> None:
    ...
    if allowed_exact_id is not None and len(matches) == 1 and int(matches[0].get("id", -1)) == allowed_exact_id:
        return
    fail("duplicate or conflicting Unit 13 version already exists in the concept lineage")
...
def anonymous_readback(client: httpx.Client, record: dict[str, Any], modern: dict[str, Any], local: dict[str, dict[str, Any]], order: list[str]) -> list[dict[str, Any]]:
    if not exact_public(record, modern, local, order):
        fail("public Unit 13 record is not an exact metadata/file match")
...
def publication_receipt(action: str, record: dict[str, Any], legacy: dict[str, Any], order: list[str], public_order: list[str], files: list[dict[str, Any]], authenticated: bool, draft: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        "schema_version": 1,
        "workflow": "o011-publish-zenodo-unit13-rdm-v1",
        "status": "pass",
...
def main() -> int:
    ...
    with httpx.Client(trust_env=False, follow_redirects=True, timeout=300, headers={"User-Agent": "O011-unit13-public-check/1.0"}) as public:
        ...
        headers = {"Authorization": f"Bearer {token}", "Accept": API_MEDIA, "User-Agent": "O011-unit13-publisher/1.0"}
        ...
        try:
            ...
            if concept_drafts:
                ...
                origin = "created_new_version_from_exact_unit10_predecessor"
            ...
            api_json(auth, "PUT", draft_url, (200,), "initial Unit 13 metadata update", json=initial, headers={"Content-Type": "application/json", "Accept": API_MEDIA})
            ...
            api_json(auth, "PUT", draft_url, (200,), "final Unit 13 metadata update", json=final, headers={"Content-Type": "application/json", "Accept": API_MEDIA})
            ...
            if projection({"metadata": verified.get("metadata") or {}}) != projection({"metadata": modern}):
                fail("final draft metadata differs from the approved Unit 13 metadata")
            ...
            ensure_no_duplicate_public_version(public, PREDECESSOR_ID)
            published = api_json(auth, "POST", f"{draft_url}/actions/publish", (201, 202), "Unit 13 draft publication")
        ...
        if recovered is None:
            fail("published Unit 13 record was not visible as an exact anonymous byte/metadata match")
