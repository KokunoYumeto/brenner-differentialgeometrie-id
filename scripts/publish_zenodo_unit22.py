#!/usr/bin/env python3
"""Publish the verified Unit 22 checkpoint to the existing Zenodo concept.
...
and public predecessor record 22134954.  It then resumes at most one
...
PREDECESSOR_DOI = "10.5281/zenodo.22134954"
PREDECESSOR_VERSION = "2026.08.26-unit19"
PREDECESSOR_TITLE = "Geometri Diferensial dan Manifold Mulus — Edisi Bahasa Indonesia (Batas Unit 19)"
PREDECESSOR_READBACK_REL = "qa/unit-19/ZENODO_PUBLIC_READBACK_RECEIPT.json"
PREDECESSOR_READBACK_SHA256 = (
    "7ff16486bbaa6867c5f85ff7761f03244b9bc683d57719f2fa26c8e0f88ca216"
)
PREDECESSOR_ORDER = [
...
RELEASE_DIR = Path("output/release-unit22")
PREPARATION_WORKFLOW = "o011-prepare-release-unit22-v1"
INTEGRITY_WORKFLOW = "o011-verify-source-package-unit22-v1"
...
PRIVATE_RE = re.compile(
    # ``/home/.../`` is a deliberate redacted placeholder in the durable
    # provenance note for an upstream SVG, not a local filesystem locator.
    ...
    try:
        with zipfile.ZipFile(archive_path, "r") as archive:
            if archive.testzip() is not None:
                fail("Unit 22 source archive failed its CRC test")
            entries = safe_archive_entries(archive, "Unit 22 source archive")
            if any(name.endswith("/00_control/PRIVATE_LOCAL_LOCATORS.md") or name == "00_control/PRIVATE_LOCAL_LOCATORS.md" for name in entries):
                fail("Unit 22 source archive contains the private-locator control")

            anchor_suffix = "00_control/GOAL_AND_WORKFLOW.md"
            anchor = unique_suffix(entries, anchor_suffix, "Unit 22 source archive")
            ...
            for path in controls:
                ...
                if member not in entries:
                    fail(f"Unit 22 source archive lacks durable control {path.name}")
                payload = archive.read(entries[member])
                text = payload.decode("utf-8", errors="strict")
                if PRIVATE_RE.search(text) or SECRET_RE.search(text) or local_profile_name_present(text):
                    fail(f"Unit 22 source archive durable control is not public-safe: {path.name}")
            ...
            for suffix in required:
                ...
                if member not in entries:
                    fail(f"Unit 22 source archive lacks required dependency {suffix}")
...
def verify_integrity_receipt(
    ...
    if (
        ...
        fail("source-package integrity receipt is not the passing Unit 22 workflow")
    ...
    if not identity_occurs(outer_html, html):
        fail("source-package integrity receipt does not bind the staged HTML ZIP")
    ...
    for item in clean:
        ...
        if not identity_occurs(rebuilt.get("pdf"), pdf):
            fail("a clean rebuild does not bind the validated Unit 22 PDF bytes")
        if rebuilt.get("html_tree_sha256") != outer_html.get("tree_sha256"):
            fail("a clean rebuild HTML tree does not bind the staged HTML ZIP inventory")
        outputs.append(rebuilt)
    if outputs[0] != outputs[1]:
        fail("source-package clean rebuild outputs differ across cycles")
...
def local_payload(
    ...
    if (
        ...
        fail("release-preparation receipt is not the passing local-only Unit 22 workflow")
    ...
    if not isinstance(rows, list) or [row.get("filename") for row in rows if isinstance(row, dict)] != EXPECTED_ORDER:
        fail("release-preparation receipt does not declare the exact reader-first Unit 22 inventory")
    if (
        stage.get("lineage")
        != {
        ...
        fail("release-preparation receipt has the wrong Unit 22 lineage or reader-first order")

    release = root / RELEASE_DIR
    if not release.is_dir():
        fail("Unit 22 release directory is absent")
    loose = sorted(path.name for path in release.iterdir() if path.is_file())
    if loose != sorted(EXPECTED_ORDER):
        fail("Unit 22 release directory contains a missing or extra loose file")
    ...
    if (
        ...
        fail("Unit 22 public payload exceeds 500 MB or its bound total changed")
    pdf_identity = {key: local[PDF_NAME][key] for key in ("bytes", "sha256")}
    if not identity_occurs(stage.get("input_bindings"), pdf_identity):
        fail("Unit 22 stage does not bind its PDF to the validated reader gate")
    ...
    if (
        ...
        fail("Unit 22 stage lacks a passing privacy scan")
    if not isinstance(stage.get("deterministic_archives"), dict) or stage["deterministic_archives"].get("status") != "pass":
        fail("Unit 22 stage lacks passing deterministic-archive evidence")
...
def metadata_pair(path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if set(legacy) != required:
        fail("Unit 22 metadata schema is not exact")
    if (
        legacy.get("title") != TITLE
        or legacy.get("version") != VERSION
        or legacy.get("publication_date") != PUBLICATION_DATE
    ):
        fail("Unit 22 title, version, or publication date is not exact")
    ...
    if not isinstance(description, str) or any(text not in description for text in required_description):
        fail("Unit 22 description lacks exact scope, rights, or provenance disclosure")
    if (
        ...
        fail("Unit 22 description duplicated the model identifier or exposed private material")
...
def predecessor_ok(record: dict[str, Any]) -> None:
    ...
    if (
        ...
        fail("Zenodo predecessor is not the exact published Unit 19 boundary")
...
def ensure_no_duplicate_version(
    ...
    if allowed_exact_id is not None and len(matches) == 1 and record_id(matches[0]) == allowed_exact_id:
        return
    fail("duplicate or conflicting Unit 22 version already exists")
...
def verify_predecessor_boundary(root: Path, predecessor: dict[str, Any]) -> dict[str, Any]:
    ...
    if not receipt_path.is_file() or digest(receipt_path) != PREDECESSOR_READBACK_SHA256:
        fail("exact Unit 19 public-readback receipt is absent or changed")
    receipt = load_object(receipt_path, "Unit 19 public-readback receipt")
    receipt_files = receipt.get("files")
    if (
        receipt.get("schema_version") != 1
        or receipt.get("workflow") != "o011-independent-zenodo-unit19-public-readback-v1"
        or receipt.get("status") != "pass"
    ...
    if (
        ...
        fail("public predecessor inventory/order/preview differs from the exact Unit 19 proof")
    ...
    for name in PREDECESSOR_ORDER:
        if (
            ...
            fail(f"public predecessor file identity differs from Unit 19 proof: {name}")
    return {
...
def anonymous_readback(
    ...
    if not exact_public(record, modern, local, order):
        fail("public Unit 22 record is not an exact metadata/file match")
...
def publication_receipt(
    ...
    result: dict[str, Any] = {
        "schema_version": 1,
        "workflow": "o011-publish-zenodo-unit22-v1",
        "status": "pass",
...
def main() -> int:
    ...
    public_headers = {"User-Agent": "O011-unit22-public-check/1.0", "Accept": API_MEDIA}
    with httpx.Client(trust_env=False, follow_redirects=True, timeout=300, headers=public_headers) as public:
        ...
        headers = {
            ...
            "Accept": API_MEDIA,
            "User-Agent": "O011-unit22-publisher/1.0",
        }
        ...
        try:
            ...
            if (
                ...
                fail("existing draft is not the exact inherited Unit 19 state or exact Unit 22 target")
            initial = {
                "metadata": modern,
            ...
            api_json(
                ...
                draft_url,
                (200,),
                "initial Unit 22 metadata update",
                json=initial,
                headers={"Content-Type": "application/json", "Accept": API_MEDIA},
            )
            draft = api_json(auth, "GET", draft_url, (200,), "Unit 22 draft read")
            if record_id(draft) != draft_id or concept_id(draft) != str(CONCEPT_ID):
                fail("Unit 22 draft escaped the expected concept")
            ...
            api_json(
                ...
                draft_url,
                (200,),
                "final Unit 22 metadata update",
                json=final,
                headers={"Content-Type": "application/json", "Accept": API_MEDIA},
            )
            verified = api_json(auth, "GET", draft_url, (200,), "final Unit 22 draft verification")
            if projection({"metadata": verified.get("metadata") or {}}) != projection({"metadata": modern}):
                fail("final Unit 22 draft metadata differs from the approved metadata")
            verified_entries = draft_files(verified)
            if set(verified_entries) != set(order) or len(verified_entries) != len(order):
                fail("final Unit 22 draft file set differs from the exact staged inventory")
            for name in order:
                if (
                    verified_entries[name].get("size") != local[name]["bytes"]
                    or checksum_md5(verified_entries[name]) != local[name]["md5"]
                ):
                    fail(f"final Unit 22 draft file identity mismatch: {name}")
            ...
            if (
                ...
                fail("final Unit 22 draft lost reader-first preview/order semantics")

            latest_before = latest(public, predecessor)
            if record_id(latest_before) != PREDECESSOR_ID:
                fail("predecessor ceased to be latest before Unit 22 publication")
            ensure_no_duplicate_version(public, PREDECESSOR_ID)
            published = api_json(
                auth,
                "POST",
        ...
        if recovered is None:
            fail("Unit 22 publication did not become an exact anonymous metadata/byte match")
